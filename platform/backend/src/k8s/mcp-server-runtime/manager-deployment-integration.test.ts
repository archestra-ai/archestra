...
describe("McpServerRuntimeManager ↔ K8sDeployment hibernation seam", () => {
  ...
  test("round trip on one physical deployment: hibernate at 2 replicas, wake back to 2", async ({
    ...
    // The two patches that change the replica count carry a
    // compare-and-swap precondition; the annotation removal deliberately does
    // not (deleting keys is idempotent, and a precondition there could only
    // strand a woken deployment mid-wake).
    ...
    expect(completeWakePatch.metadata?.resourceVersion).toBeUndefined();
  });
