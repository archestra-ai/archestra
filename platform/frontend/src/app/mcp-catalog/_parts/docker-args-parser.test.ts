...
describe('parseDockerArgsToLocalConfig', () => {
	...
	it('should return null for non-Docker commands', () => {
		const result = parseDockerArgsToLocalConfig('npx', ['-y', '@modelcontextprotocol/server'], undefined);

		expect(result).toBeNull();
	});

	it('should return null when no docker_image is provided', () => {
		const result = parseDockerArgsToLocalConfig('docker', ['run', '-i', '--rm', 'some-image'], undefined);
	...
	it('should handle undefined args', () => {
		const result = parseDockerArgsToLocalConfig('docker', undefined, 'mcp/grafana');
	...
	it('should handle command with single flag argument', () => {
		const result = parseDockerArgsToLocalConfig('docker', ['run', '-i', '--rm', 'mcp/grafana', '--help'], 'mcp/grafana');
