...
import {
  useAgentExecutionShare,
  useShareAgentExecution,
  useUnshareAgentExecution,
...
type ShareVisibility =
  | "private"
  | archestraApiTypes.ShareAgentExecutionData["body"]["visibility"];

export function ShareAgentExecutionDialog({
  taskId,
  open,
  onOpenChange,
}: {
  ...
  const { data: share, isLoading } = useAgentExecutionShare(
    open ? taskId : undefined,
  );
  const shareMutation = useShareAgentExecution();
  const unshareMutation = useUnshareAgentExecution();
  ...
  const shareLink = `${window.location.origin}/chat/executions/${taskId}`;
  ...
  const handleSave = useCallback(async () => {
    ...
    toast.success("Execution visibility updated and share link copied");
  }, [
    isLoading,
    isPending,
    isShared,
