...
/**
 * Emitted by `assertTransition()` (hibernation-state-machine.ee.ts) when a
...
test.describe("MCP hibernation - administrator recovery", () => {
  ...
  test("two concurrent hard resets perform exactly one teardown", async ({
    ...
    // The second caller is handed the report of the reset it joined, so the
    // two answers are the same account of the same teardown — every field of
    // it. Two independent cycles would have to agree on the teardown
    // discriminant as well ("terminated" vs "force-killed" vs "unverified"),
    // which the second cycle, tearing down a deployment the first had just
    // rebuilt, has no reason to.
    expect(
      secondResult,
      "the second caller must be answered with the reset it joined",
    ).toEqual(firstResult);
    // ...and that shared report describes THIS deployment, so the equality
    // above cannot be two copies of something unrelated.
    expect(firstResult.mcpServerId).toBe(serverId);
    expect(firstResult.physicalDeployment).toBe(
      `${MCP_SERVER_NAMESPACE}/${deploymentName}`,
    );
    expect(firstResult.resetServerIds).toContain(serverId);
    expect(
      firstResult.rebuild,
      `the rebuilt deployment did not come up: ${JSON.stringify(firstResult.rebuild)}`,
    ).toEqual({ outcome: "ready" });
