/**
 * x.ai LLM Proxy Adapter - OpenAI-compatible
 *
 * x.ai uses an OpenAI-compatible API at https://api.x.ai/v1
 * This adapter reuses OpenAI's logic with x.ai-specific configuration.
 *
  ...
  implements LLMRequestAdapter<XaiRequest, XaiMessages>
{
  ...
  convertToolResultContent(messages: XaiMessages): XaiMessages {
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to xai format
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[XaiAdapter] Stripped browser tool results from messages",
        );
      }
    }
...
function convertMcpImageBlocksToXai(
  ...
  for (const item of content) {
    ...
    if (isMcpImageBlock(item)) {
      ...
      logger.info(
        ...
        "[XaiAdapter] Converting MCP image block to xai format",
      );
  ...
  logger.info(
    ...
    "[XaiAdapter] Converted MCP content to xai format",
  );
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[XaiAdapter] Stripped image blocks from tool result",
    );
  }
...
async function convertToolResultsToToon(
  ...
  // Use tiktoken tokenizer (same as OpenAI) for xai
...
> = {
  ...
  createClient(
    ...
    // Use OpenAI SDK with x.ai base URL (OpenAI-compatible API)
  ...
  async executeStream(
    ...
    const stream =
      await xaiClient.chat.completions.create(xaiRequest);
  ...
  extractErrorMessage(error: unknown): string {
    // OpenAI SDK error structure (same for xai)
