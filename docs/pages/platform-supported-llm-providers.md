...
- **Deployment Discovery**: For Entra ID configurations, Archestra first tries Azure deployment discovery. If the inference endpoint cannot list deployments, Archestra uses Azure management APIs to find the Cognitive Services account and list its deployments before falling back to the available model catalog. Grant the workload identity `Reader` on the Azure AI Services resource or subscription when exact deployment discovery is required.
