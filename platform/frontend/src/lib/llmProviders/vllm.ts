...
// Define more precise types for Vllm parts since the generated types use union discrimination
type VllmFunctionCallPart = {
  thought?: boolean;
...
function hasFunctionResponse(
  part: unknown,
): part is VllmFunctionResponsePart {
  return (
    typeof part === "object" &&
    part !== null &&
...
function hasFunctionCall(part: unknown): part is VllmFunctionCallPart {
  return (
    typeof part === "object" &&
    part !== null &&
...
function hasText(part: unknown): part is VllmTextPart {
  return (
    typeof part === "object" &&
    part !== null &&
    "text" in part &&
    typeof (part as VllmTextPart).text === "string"
  );
}

function hasInlineData(part: unknown): part is VllmInlineDataPart {
  return (
    typeof part === "object" &&
    part !== null &&
    "inlineData" in part &&
...
function hasFileData(part: unknown): part is VllmFileDataPart {
  return (
    typeof part === "object" &&
    part !== null &&
    "fileData" in part &&
...
class VllmGenerateContentInteraction implements InteractionUtils {
  private request: archestraApiTypes.VllmGenerateContentRequest;
  private response: archestraApiTypes.VllmGenerateContentResponse;
  modelName: string;

  constructor(interaction: Interaction) {
    this.request =
      interaction.request as archestraApiTypes.VllmGenerateContentRequest;
    this.response =
      interaction.response as archestraApiTypes.VllmGenerateContentResponse;
  ...
  private mapToUiMessage(
    content:
      | archestraApiTypes.VllmGenerateContentRequest["contents"][number]
      | {
          role: "model";
          content: archestraApiTypes.VllmGenerateContentResponse["candidates"][number]["content"];
        },
    ...
    for (const part of contentParts) {
      if (hasText(part) && part.text) {
        ...
        parts.push({
          ...
          toolCallId: part.functionCall.id || `vllm-${Date.now()}`,
              ...
              if (
                ...
                if (
                  ...
                  toolCallParts.push({
                    ...
                    toolCallId: functionCallId || `vllm-${Date.now()}`,
...
export default VllmGenerateContentInteraction;
