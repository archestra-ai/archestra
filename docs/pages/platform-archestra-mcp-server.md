...
| `oauthConfig` | `object` | No | [Remote] OAuth configuration for the server. client_secret cannot be set when editing an existing server — the user manages it in the registry UI. |
...
| `environment[].value` | `string` | No | Literal environment variable value. Managed secret values (type "secret" without promptOnInstallation) cannot be set when editing an existing server — the user manages those in the registry UI. |
