 ...
 *
 * This adapter is used by the legacy v1 OpenAI route handler:
 * - src/routes/proxy/cerebras.ts
 *
 ...
 * - src/routes/proxy/adapterV2/cerebras.ts
 *
 * V2 routes are located at:
 * - src/routes/proxy/routesv2/cerebras.ts
 *
...
export function toCommonFormat(messages: OpenAiMessages): CommonMessage[] {
  logger.debug(
    ...
    "[adapters/cerebras] toCommonFormat: starting conversion",
  );
    ...
    if (message.role === "tool") {
      ...
      if (toolName) {
        logger.debug(
          ...
          "[adapters/cerebras] toCommonFormat: found tool message",
        );
  ...
  logger.debug(
    ...
    "[adapters/cerebras] toCommonFormat: conversion complete",
  );
...
export function applyUpdates(
  ...
  logger.debug(
    ...
    "[adapters/cerebras] applyUpdates: starting",
  );

  if (updateCount === 0) {
    logger.debug("[adapters/cerebras] applyUpdates: no updates to apply");
    return messages;
  }
  ...
  const result = messages.map((message) => {
    if (message.role === "tool" && updates[message.tool_call_id]) {
      ...
      logger.debug(
        ...
        "[adapters/cerebras] applyUpdates: applying update to tool message",
      );
      return {
        ...message,
  ...
  logger.debug(
    ...
    "[adapters/cerebras] applyUpdates: complete",
  );
  return result;
}
