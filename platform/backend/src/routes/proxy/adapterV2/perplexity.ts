 ...
 *
 * Perplexity uses an OpenAI-compatible API at https://api.perplexity.com
 * This adapter reuses OpenAI's logic with Perplexity-specific configuration.
 *
  ...
  implements LLMRequestAdapter<PerplexityRequest, PerplexityMessages>
{
  ...
  convertToolResultContent(messages: PerplexityMessages): PerplexityMessages {
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to Perplexity format
      const convertedContent = convertMcpImageBlocksToPerplexity(message.content);
      if (!convertedContent) {
        return message;
      }
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[PerplexityAdapter] Stripped browser tool results from messages",
        );
      }
    }
  ...
  logger.info(
    {
      ...
      imageBlocks: perplexityContent.filter((b) => b.type === "image_url").length,
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[PerplexityAdapter] Stripped image blocks from tool result",
    );
  }
...
async function convertToolResultsToToon(
  ...
  // Use tiktoken tokenizer (same as OpenAI) for Perplexity
...
> = {
  ...
  extractErrorMessage(error: unknown): string {
    // OpenAI SDK error structure (same for Perplexity)
