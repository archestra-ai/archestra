...
A pod placed on a node without the image still pulls it. A node the autoscaler just added starts empty, for example. [Image pre-pulling](#image-pre-pulling) closes that gap.
...
Choose **Restart pods with a fresh image** on the registry entry to move a server onto the current image. That rollout goes to the container registry. Pre-pulling re-pulls the refreshed image on every node as well, so the fleet cache carries the new image — not the one it held before. The next restart or reinstall returns the server to the cached image.

### Image Pre-Pulling

Pre-pulling puts every MCP server image on every node before a wake needs it. Fresh nodes get the images as well. A wake then finds its image locally, wherever the scheduler places the pod.

Archestra runs a DaemonSet named `<release>-mcp-image-prepuller` in the platform's own namespace. It pulls each distinct image of your self-hosted MCP deployments. Images that exist only on the node are skipped, because no registry serves them.

The DaemonSet needs room on every node — one pod slot, and a small CPU and memory request. It uses the same node selector and tolerations as your MCP servers.

A private-registry image needs its pull secret in that same namespace. Kubernetes never reads a Secret from another namespace. Archestra puts a server's pull secret in the namespace of its environment. Pre-pulling therefore skips a private image from any other environment — the log names it. That server still wakes, and the wake pulls from your registry.

Pre-pulling runs only while idle hibernation is on. Set `ARCHESTRA_ORCHESTRATOR_MCP_IMAGE_PREPULL_ENABLED` to `false` to turn it off and keep hibernation. Turning it off also removes the DaemonSet. See [Environment Variables](/docs/platform-deployment#mcp-server-orchestrator).

Pre-pulling never blocks a wake, an install, or a tool call. Archestra logs a failure and carries on. If the platform's Kubernetes role lacks DaemonSet permissions, the log names the rule to add.

One node that cannot pull an image never holds back the other nodes. The DaemonSet updates all of them at once.
