...
import {
  ...
  useDeleteAgentRuntimeCredential,
  useSetAgentRuntimeCredential,
...
export function AgentRuntimeCredentialCard({
  agentId,
  credentials,
  readOnly = false,
  editHref,
  ...
  const setCredential = useSetAgentRuntimeCredential(agentId);
  const deleteCredential = useDeleteAgentRuntimeCredential(agentId);
  const deleteConnection = useDeleteRuntimeCredentialConnection();
  ...
  if (useExternalSecretsManager) {
    return (
      <ExternalSecretReferenceDialog
        ...
        description="Select the Vault secret for this Agent's Agent Runtime."
        onClose={onClose}
        onConfirm={onSave}
      />
    );
