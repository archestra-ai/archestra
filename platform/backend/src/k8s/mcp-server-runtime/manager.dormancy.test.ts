...
describe("McpServerRuntimeManager.isDeploymentDormant", () => {
  ...
  test("observed sleep remains dormant after idle hibernation is turned off", async ({
    makeMcpServer,
  }) => {
    const mcpServer = await makeMcpServer({ deploymentName: DEPLOYMENT_NAME });
    const alias = loadAlias(mcpServer as McpServer, live);
    ...
    // Disabling future sleep does not wake existing zero-replica deployments.
    // Passive background paths must keep skipping it until demand wakes it.
    expect(manager.isDeploymentDormant(mcpServer.id)).toBe(true);
  });
});
