...
/**
 * Report whether a connection setup the caller created has been consumed (its one-time script fetched and run). Lets the /connection page flip the command step to a confirmed state.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * None (no additional RBAC permission required)
 */
export const getConnectionSetupStatus = <ThrowOnError extends boolean = false>(options: Options<GetConnectionSetupStatusData, ThrowOnError>) => (options.client ?? client).get<GetConnectionSetupStatusResponses, GetConnectionSetupStatusErrors, ThrowOnError>({ url: '/api/connection-setups/{id}', ...options });
