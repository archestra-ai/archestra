 ...
 * Groq API Types
 *
 * Groq exposes an OpenAI-compatible API server, so we re-export OpenAI schemas.
  ...
  .object({
    ...
    message: z
      .object({
        ...
        // Groq-specific: reasoning field for models that support it
...
export const ChatCompletionsHeadersSchema = z.object({
  ...
  authorization: z
    .string()
    .optional()
    .describe("Bearer token for Groq (often not required)")
