...
describe("GET /api/projects/:id/runs (project:read-all)", () => {
  ...
  test("a member without read-all sees only runs they started", async () => {
    ...
    expect(response.json<RunItem[]>()).toEqual([
  ...
  test("an admin sees all project runs read-only", async ({
    makeUser,
    makeMember,
  }) => {
  ...
  test("a custom project read-all role sees every run", async ({
    makeUser,
    makeMember,
    makeCustomRole,
  }) => {
