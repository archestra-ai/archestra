    ...
    logger.info(
      {
        ...
        rawParts: JSON.stringify(candidate.content.parts).substring(0, 500),
      },
      "[GeminiStreamAdapter] Processing chunk parts",
    );
    ...
    for (const part of candidate.content.parts) {
      ...
      if ("inlineData" in part && part.inlineData) {
        logger.info(
          ...
          "[GeminiStreamAdapter] Found inlineData part (image)",
        );
...
> = {
  ...
  async execute(
    ...
    // Log SDK params to debug image generation config
    logger.info(
      {
        model,
        hasGenerationConfig: !!request.generationConfig,
        responseModalities: request.generationConfig?.responseModalities,
        sdkConfigResponseModalities: (
          sdkParams.config as Record<string, unknown>
        )?.responseModalities,
      },
      "[GeminiAdapter] SDK params for non-streaming request",
    );
    ...
    // Log raw SDK response to debug image generation
      ...
      logger.info(
        {
          ...
          hasInlineData: parts.some((p) => "inlineData" in p && p.inlineData),
        },
        "[GeminiAdapter] Raw SDK non-streaming response received",
      );
    }
  ...
  async executeStream(
    ...
    logger.info(
      ...
      "[GeminiAdapter] SDK params for streaming request",
    );
    ...
    return {
      [Symbol.asyncIterator]: async function* () {
        for await (const chunk of streamingResponse) {
          // Log raw SDK chunk to debug image generation
          ...
          if (parts) {
            logger.info(
              ...
              "[GeminiStreamAdapter] Raw SDK chunk received",
            );
          }
          yield chunk;
        }
