...
1. A running Archestra stack (`tilt up` from `platform/`). Verify:
  ...
  2. OpenAPPA enabled on the backend: `ARCHESTRA_OPENAPPA_ENABLED=true` in
    ...
    off, `http://localhost:3000/openappa` 404s and the proxy gates nothing. The
    offer signing key needs no configuration: it derives from the auth secret,
    which Tilt defaults for local development.
