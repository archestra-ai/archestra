...
| Tool | Description | Required RBAC Permission |
|------|-------------|--------------------------|
| `scaffold_app` | Create a new interactive app (dashboard, form, tracker, game, or any custom UI). | `app:create` |
...
| `read_app` | Return an app's stored source before edit_app whenever it is not already in context. | `app:read` |
| `edit_app` | Change an app's HTML. | `app:update` |
...
|-----------|------|----------|-------------|
| `appId` | `string` | Yes | The app id. |
| `format` | `"html" \| "sections"` | No | How to read the app. "html" (default) returns the full stored document, windowable with offset/limit. "sections" returns a managed-sections app's authored title/css/body/javascript values (all four, or a single windowed section); it errors on a non-managed document. |
| `section` | `"title" \| "css" \| "body" \| "javascript"` | No | With format "sections": return only this one section (and window it with offset/limit). Omit to return all four section values whole. |
...
| `limit` | `integer` | No | Maximum number of characters to return, starting at offset. Omitted reads to the end; 0 returns no content, just the size metadata. |
...
| `sections` | `object` | No | The default path for a managed-sections app: edit its title/css/body/javascript by value, no HTML boilerplate. Each supplied section replaces (string) or patches ({ edits }) only that section; omitted sections are unchanged. Pass exactly one of sections, edits, or replacementHtml. |
| `sections.title` | `string` | No | The document title as plain text (replacement only, not HTML). |
| `sections.css` | `string \| object` | No | The app's CSS: a string to replace the whole section, or { edits } to str_replace-patch it. |
| `sections.css.edits` | `object[]` | Yes | str_replace edits applied in order to this section's source; the whole set is atomic. |
| `sections.css.edits[].old_str` | `string` | Yes | Exact text to replace within this section's current source; must occur exactly once (add surrounding context to disambiguate). |
| `sections.css.edits[].new_str` | `string` | Yes | Replacement text (may be empty to delete). |
| `sections.body` | `string \| object` | No | The app's body HTML (inside <main id="app">): a string to replace the whole section, or { edits } to patch it. |
| `sections.body.edits` | `object[]` | Yes | str_replace edits applied in order to this section's source; the whole set is atomic. |
| `sections.body.edits[].old_str` | `string` | Yes | Exact text to replace within this section's current source; must occur exactly once (add surrounding context to disambiguate). |
| `sections.body.edits[].new_str` | `string` | Yes | Replacement text (may be empty to delete). |
| `sections.javascript` | `string \| object` | No | The app's JavaScript: a string to replace the whole section, or { edits } to patch it. |
| `sections.javascript.edits` | `object[]` | Yes | str_replace edits applied in order to this section's source; the whole set is atomic. |
| `sections.javascript.edits[].old_str` | `string` | Yes | Exact text to replace within this section's current source; must occur exactly once (add surrounding context to disambiguate). |
| `sections.javascript.edits[].new_str` | `string` | Yes | Replacement text (may be empty to delete). |
