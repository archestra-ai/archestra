/**
 * Fails the build if a model query on a soft-deletable table is missing
 * `notDeleted` (or an `includeDeleted` opt-in).
 *
 * Blind spots: `db.query.<table>` relational queries, raw `db.execute()`,
 * statements spanning more than WINDOW_LINES lines, re-export aliases.
 * For an intentional bypass, put `// soft-delete:` in the same window.
 */
...
function listSoftDeletableTables(): Set<string> {
  ...
  for (const file of schemaFiles) {
    ...
    const isSoftDeletable =
      src.includes("softPgTable(") || src.includes("softDeleteColumns");
    if (!isSoftDeletable) continue;

    const tableMatches = src.matchAll(
      /(?:const|export\s+const)\s+(\w+)\s*=\s*(?:softPgTable|pgTable)\(/g,
    );
