 ...
 * This adapter reuses OpenAI's request/response/stream adapters
 * with x.ai-specific configuration (baseUrl, api key behavior).
 *
...
> = {
  ...
  createClient(
    ...
    if (!apiKey) {
      throw new Error("API key required for x.ai (Grok)");
    }
