...
export function makeSwapAgentPokeText(agentName: string): string {
  return `${SWAP_AGENT_POKE_PREFIX}${agentName}${SWAP_AGENT_POKE_AGENT_NAME_SUFFIX} If you have the swap_to_default_agent tool and you don't have the right tools to fulfill the request, use it immediately — write a brief message explaining why you are switching back, then call swap_to_default_agent.)`;
}
