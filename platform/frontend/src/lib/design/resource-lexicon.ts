...
/**
 * The one Created value, for every surface that shows a record's provenance.
 *
 * The date and nothing else, so a column of these is scannable and every
 * surface can use the same call. Who the record belongs to is a fact of its
 * own and gets its own field on the Details card: appending "by Admin" here
 * as well named the same person twice on the same card. Pair it with
 * `FIELD_LABEL.created` — the value never repeats the word "Created".
 */
export function formatCreated({ createdAt }: { createdAt: string }): string {
  return formatDate({ date: createdAt, dateFormat: "PP" });
}
