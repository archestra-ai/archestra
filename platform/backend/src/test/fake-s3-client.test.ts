...
describe("FakeS3Client", () => {
  test("put then get round-trips bytes", async () => {
    ...
    await s3.send(new PutObjectCommand({ Bucket: B, Key: "a/x.txt", Body: Buffer.from("hi") }));
    const res: any = await s3.send(new GetObjectCommand({ Bucket: B, Key: "a/x.txt" }));
  ...
  test("conditional put on an existing key throws 412 PreconditionFailed", async () => {
    const s3 = new FakeS3Client();
    await s3.send(new PutObjectCommand({ Bucket: B, Key: "a/x.txt", Body: Buffer.from("v1") }));
    await expect(
      s3.send(new PutObjectCommand({ Bucket: B, Key: "a/x.txt", Body: Buffer.from("v2"), IfNoneMatch: "*" })),
  ...
  test("list with delimiter splits direct children from nested prefixes", async () => {
    ...
    expect((res.Contents ?? []).map((c: any) => c.Key)).toEqual(["a/top.txt"]);
    expect((res.CommonPrefixes ?? []).map((p: any) => p.Prefix)).toEqual(["a/sub/"]);
  });
  ...
  test("list paginates Contents via the continuation token", async () => {
    ...
    s3.putRaw("a/2.txt", "x");
    s3.putRaw("a/3.txt", "x");
    const p1: any = await s3.send(new ListObjectsV2Command({ Bucket: B, Prefix: "a/", Delimiter: "/" }));
    expect(p1.Contents.length).toBe(2);
    expect(p1.IsTruncated).toBe(true);
    const p2: any = await s3.send(
      new ListObjectsV2Command({ Bucket: B, Prefix: "a/", Delimiter: "/", ContinuationToken: p1.NextContinuationToken }),
    );
