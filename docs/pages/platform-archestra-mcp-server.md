...
| `run.created_at` | `string` | Yes | ISO 8601. |
| `run.state_changed_at` | `string` | Yes | ISO 8601 of the last transition. |
| `runtime_state` | `object \| null` | Yes | Latest runtime activity and actionable diagnostic for the returned task_id, when available. |
| `runtime_state.version` | `1` | Yes |  |
| `runtime_state.attemptId` | `string` | Yes |  |
| `runtime_state.sequence` | `integer` | Yes |  |
| `runtime_state.eventId` | `string` | Yes |  |
| `runtime_state.source` | `string` | Yes |  |
| `runtime_state.observedAt` | `string` | Yes |  |
| `runtime_state.activity` | `"working" \| "idle" \| "unknown"` | Yes |  |
| `runtime_state.outcome` | `"failed" \| "succeeded" \| null` | Yes |  |
| `runtime_state.diagnostic` | `object \| null` | Yes |  |
| `runtime_state.diagnostic.code` | `string` | Yes |  |
| `runtime_state.diagnostic.phase` | `"startup" \| "credentials" \| "provider" \| "tool" \| "terminal" \| "protocol"` | Yes |  |
| `runtime_state.diagnostic.message` | `string` | Yes |  |
| `runtime_state.diagnostic.resolution` | `string` | Yes |  |
| `runtime_state.diagnostic.httpStatus` | `integer` | No |  |
