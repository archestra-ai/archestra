...
describe("PUT /api/internal_mcp_catalog/:id — hibernation mode cascade", () => {
  ...
  test("a PUT that fails validation does not change runtime policy", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const { catalog, installA, installB } = await makeCatalogWithInstalls({
    ...
    // A sibling whose name the rename below collides with.
    ...
    const response = await app.inject({
      method: "PUT",
      url: `/api/internal_mcp_catalog/${catalog.id}`,
      payload: { hibernationMode: "disabled", name: "Taken Name" },
    });

    // The rename 409s — and the mode cascade must not have run on the way
    // out: a failed PUT that silently changed runtime policy would leave the
    // registry dialog reporting one thing and the sweeper doing another.
    expect(response.statusCode).toBe(409);
    const [modeA, modeB] = await Promise.all([
      McpServerModel.findById(installA.id),
      McpServerModel.findById(installB.id),
    ]);
    expect(modeA?.hibernationMode).toBe("inherit");
    expect(modeB?.hibernationMode).toBe("inherit");
  });
