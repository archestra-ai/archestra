...
import OpenAIProvider from "minimax";
import { z } from "zod";
...
import { MockOpenAIClient } from "./mock-minimax-client";
...
const openAiProxyRoutes: FastifyPluginAsyncZod = async (fastify) => {
   ...
   * - /v1/minimax/:agentId/* -> config.llm.minimax.baseUrl/* (agentId stripped if UUID)
   *  - /v1/minimax/* -> config.llm.minimax.baseUrl/* (direct proxy)
   *
  ...
  const handleChatCompletion = async (
    ...
    const openAiClient = config.benchmark.mockMode
      ...
      : new OpenAIProvider({
          ...
          fetch: getObservableFetch("minimax", resolvedAgent, externalAgentId),
        });
    ...
    try {
      ...
      const commonMessages = utils.adapters.minimax.toCommonFormat(messages);
      logger.debug(
      ...
      let filteredMessages = utils.adapters.minimax.applyUpdates(
        messages,
        toolResultUpdates,
      );
      ...
      if (shouldApplyToonCompression) {
        const { messages: convertedMessages, stats } =
          await utils.adapters.minimax.convertToolResultsToToon(
            filteredMessages,
            model,
          );
      ...
      fastify.log.info(
        ...
        "minimax proxy routes: handle chat completions: tool results compression completed",
      );
      ...
      if (stream) {
        ...
        try {
          for await (const chunk of streamingResponse) {
            ...
            if (chunk.usage) {
              tokenUsage = utils.adapters.minimax.getUsageTokens(chunk.usage);
            }
          ...
          if (tokenUsage) {
            ...
            fastify.log.info(
              ...
              "minimax proxy routes: handle chat completions: costs",
            );
          } else {
            fastify.log.warn(
        ...
        const tokenUsage = response.usage
          ? utils.adapters.minimax.getUsageTokens(response.usage)
