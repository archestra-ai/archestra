...
import { DARK_ONLY_THEMES, LIGHT_ONLY_THEMES, SUPPORTED_THEMES } from "./theme-config";
...
function generateThemeCSS(theme: ThemeItem): string {
  ...
  const isLightOnly = (LIGHT_ONLY_THEMES as readonly string[]).includes(theme.name);
  const isDarkOnly = (DARK_ONLY_THEMES as readonly string[]).includes(theme.name);
