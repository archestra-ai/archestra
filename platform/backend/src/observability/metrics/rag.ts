...
export function initializeRagMetrics(): void {
  ...
  ragDocumentsWithoutTextTotal = new client.Counter({
    ...
    help: "Documents a content sync found but could not extract any text from (scanned PDF with no text layer, unparseable file) — skipped, so invisible to search",
    labelNames: ["connector_type"],
  });
