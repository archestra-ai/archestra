...
[`query_knowledge_sources`](#query_knowledge_sources) is an exception: its output is treated as untrusted by default and is evaluated by trusted data policies (and may cause later tool calls to be blocked in sensitive contexts). You can configure its guardrails from the Tool Guardrails page at /mcp/tool-guardrails.
...
[`query_knowledge_sources`](#query_knowledge_sources) is evaluated by trusted data policies and is treated as untrusted by default.
