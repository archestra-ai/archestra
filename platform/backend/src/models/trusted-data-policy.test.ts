...
describe("TrustedDataPolicyModel", () => {
  ...
  describe("Archestra MCP server tools", () => {
    ...
    test("can explicitly trust query_knowledge_sources output via a trusted data policy", async ({
      makeAgent,
      seedAndAssignArchestraTools,
    }) => {
      const agent = await makeAgent();
      ...
      const kbTool = await ToolModel.findByName(queryToolName);
    ...
    test("can explicitly block query_knowledge_sources output via a blocking policy", async ({
      makeAgent,
      seedAndAssignArchestraTools,
    }) => {
      const agent = await makeAgent();
      ...
      expect(result.isTrusted).toBe(false);
      expect(result.shouldSanitizeWithDualLlm).toBe(false);
      expect(result.reason).toContain("Block KB output");
    });

    test("can explicitly sanitize query_knowledge_sources output via a sanitizing policy", async ({
      makeAgent,
      seedAndAssignArchestraTools,
    }) => {
      const agent = await makeAgent();
      ...
      await TrustedDataPolicyModel.create({
        ...
        conditions: [],
        action: "sanitize_with_dual_llm",
        description: "Sanitize KB output",
      });
      ...
      expect(result.reason).toContain("Sanitize KB output");
    });
