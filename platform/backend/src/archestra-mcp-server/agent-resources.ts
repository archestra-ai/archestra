...
export const CreateBaseToolArgsSchema = z
  .object({
    ...
    toolExposureMode: ToolExposureModeSchema.optional().describe(
      "How tools should be loaded for MCP clients and models. Use 'search_and_run_only' to keep the initial tool list small while letting search_tools find assigned tools and run_tool execute them. Assigned skill discovery/activation tools (list_skills, activate_skill, read_skill_file), sandbox runtime tools (run_command, download_file, upload_file) — when the code runtime is enabled and assigned — and app tools (create_app, update_app, render_app, list_apps) stay directly available in both modes.",
    ),
  })
  .strict();
