...
describe("NotionConnector", () => {
  ...
  describe("testConnection", () => {
    it("returns failure on non-OK response", async () => {
      const connector = new NotionConnector();
      vi.spyOn(
        connector as unknown as { notionFetch: unknown },
        "notionFetch",
  ...
  describe("sync — database mode (with databaseIds)", () => {
    it("yields documents from database query", async () => {
      ...
      // database query
      fetchMock.mockResolvedValueOnce(makeDatabaseQueryResponse(pages));
      // blocks for db-page-1
      fetchMock.mockResolvedValueOnce(makeBlocksResponse(["DB content 1"]));
      // blocks for db-page-2
      fetchMock.mockResolvedValueOnce(makeBlocksResponse(["DB content 2"]));
    ...
    it("paginates through database query results", async () => {
      ...
      fetchMock.mockResolvedValueOnce(makeBlocksResponse(["Content 1"]));

      // Second page of results
      fetchMock.mockResolvedValueOnce(makeDatabaseQueryResponse([page2]));
      fetchMock.mockResolvedValueOnce(makeBlocksResponse(["Content 2"]));
