/**
 * Z.ai (Zhipu AI) Interaction Handler
 *
 * Z.ai uses an OpenAI-compatible API, so this handler extends the OpenAI
 * interaction handler with Z.ai-specific types.
 *
...
// Z.ai uses OpenAI-compatible request/response types
type ZaiChatCompletionRequest = archestraApiTypes.OpenAiChatCompletionRequest;
type ZaiChatCompletionResponse = archestraApiTypes.OpenAiChatCompletionResponse;

class ZaiChatCompletionInteraction implements InteractionUtils {
  private request: ZaiChatCompletionRequest;
  private response: ZaiChatCompletionResponse;
  modelName: string;

  constructor(interaction: Interaction) {
    this.request = interaction.request as ZaiChatCompletionRequest;
    this.response = interaction.response as ZaiChatCompletionResponse;
  ...
  private mapToUiMessage(
    message:
      | ZaiChatCompletionRequest["messages"][number]
      | ZaiChatCompletionResponse["choices"][number]["message"],
  ): PartialUIMessage {
    ...
    const roleToUIRoleMap: Record<
      ZaiChatCompletionRequest["messages"][number]["role"],
...
export default ZaiChatCompletionInteraction;
