...
import { isDynamicallyAvailableArchestraTool } from "@/archestra-mcp-server/dynamic-tools";
import { filterToolNamesByPermission } from "@/archestra-mcp-server/rbac";
import { agentToolExclusionsService } from "./agent-tool-exclusions";
...
// release/1.3 predates the shared assignment-or-dynamic availability helper.
async function isArchestraToolAvailableToAgent(params: {
  toolName: string;
  agentId: string;
  userId?: string;
  organizationId?: string;
}): Promise<boolean> {
  const { tools } = await agentToolExclusionsService.getFilteredMcpToolsByAgent(
    params.agentId,
  );
  return (
    tools.some((tool) => tool.name === params.toolName) ||
    isDynamicallyAvailableArchestraTool(params)
  );
}
