...
const UNAVAILABLE_TOOL_ERROR_MESSAGE =
  "The requested tool is not available in this chat. Available tools are listed in the details below. Tool names carry their server as a prefix in the form `server__tool` (e.g. a submission tool is `server__submit_result`); the bare short name without that prefix will not match. Copy an exact name from the list for the next tool call.";
