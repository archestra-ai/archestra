...
const MOCK_STREAMING_CHUNKS: OpenRouter.Types.ChatCompletionChunk[] = [
  {
    id: "chatcmpl-mock-openrouter-123",
...
/**
 * Mock OpenRouter Client that returns immediate tool call responses
 */
export class MockOpenRouterClient {
  ...
  static setStreamOptions(options: MockStreamOptions) {
    MockOpenRouterClient.streamOptions = options;
  }
  ...
  static resetStreamOptions() {
    MockOpenRouterClient.streamOptions = {};
  }
  ...
  async chatCompletionsStream(
    _request: OpenRouter.Types.ChatCompletionsRequest,
  ): Promise<AsyncIterable<OpenRouter.Types.ChatCompletionChunk>> {
    return {
      [Symbol.asyncIterator]() {
        ...
        return {
          async next() {
            if (
              MockOpenRouterClient.streamOptions.interruptAtChunk !== undefined &&
              index === MockOpenRouterClient.streamOptions.interruptAtChunk
            ) {
