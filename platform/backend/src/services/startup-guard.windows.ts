...
/**
 * PowerShell counterpart of the client-agnostic POSIX startup guard renderer in
 * startup-guard.ts — same behavior contract, driven by the same
 * {@link StartupGuardClient} descriptors (Claude Code, Codex, Copilot CLI),
 * expressed for Windows PowerShell 5.1 and PowerShell 7+:
 *
 ...
 *   the client exits; an unreachable platform gets the 15s capped-backoff retry
 ...
 *   re-check the remote; once nothing connected is left to check, the guard
 *   uninstalls itself entirely (script, skip file, profile wrapper blocks);
 * - never blocks a launch: non-interactive runs (redirected console, or a
 *   descriptor's one-shot args) only warn on stderr, and the profile wrapper
 *   always runs the real binary afterward. The client's disable env var (e.g.
 *   ARCHESTRA_CLAUDE_GUARD=0) turns it off.
 *
 * The guard is installed at ~/`client.psScriptRelpath` and hooked via a
 * marker-delimited `<binary>` function in profile.ps1 of every PowerShell
 * edition present (Windows PowerShell and PowerShell 7 keep separate profile
 * directories).
 */

/**
 * The standalone guard script body (the .ps1 file) for a given client.
 *
 * @public — consumed by the install section below and exercised directly by
 * the unit tests, which knip --production ignores.
 */
export function renderStartupGuardPowerShell(
  ...
  return `# ${ctx.appName} pre-loader for ${client.label} (PowerShell) — generated by the ${ctx.appName} /connection page.
...
$GuardPath = Join-Path $env:USERPROFILE ${psq(client.psScriptRelpath)}
...
$SkipFile = Join-Path $env:USERPROFILE ${psq(client.skipRelpath)}
$DisconnectedKinds = @()
...
function Remove-ArchGuard {
  ...
  foreach ($profilePath in ($profilePaths | Select-Object -Unique)) {
    ...
    foreach ($line in @(Get-Content -Path $profilePath)) {
      if ($line -eq ${psq(client.markerStart)}) { $skip = $true; continue }
      if ($line -eq ${psq(client.markerEnd)}) { $skip = $false; continue }
...
$Interactive = $true
foreach ($a in $args) { if (${client.nonInteractiveArgPatterns.map((p) => `$a -eq ${psq(p)}`).join(" -or ")}) { $Interactive = $false } }
try {
...
# down-prompt disconnect and the reconfigure menu so the two can't drift. The
...
${client.windows.mcpDisconnect}
    ...
    ctx.skills
      ? `
    'skills' {
${client.windows.skillsDisconnect}
    }`
      : ""
  }${
    ctx.proxy
...
function Disconnect-ArchRemote([string]$Kind, [string]$Label) {
  ...
  if ($Kind -eq 'proxy') {
    Clear-ArchLine
    Write-Arch ${psq(`  ${client.windows.proxyDisconnectNote(ctx)}`)} DarkGray
  }`
      : ""
  }
}
...
function Show-ArchMenuFooter([int]$count, [int]$baseTop) {
  $text = '  Press 1-' + $count + ' to disconnect a resource from ${client.promptName} · [Esc] Done'
  if ($UseVt) {
...
function Show-ArchDownSummaryPrompt($downRemotes) {
  ...
  if ($downRemotes.Count -eq 1) {
    $q = 'Disconnect ' + $downRemotes[0].FailName + ' from ${client.promptName} now? (Y/n) '
  } else {
    $q = 'Disconnect all ' + $downRemotes.Count + ' unreachable resources from ${client.promptName} now? (Y/n) '
  }
  ...
  if ($downRemotes.Count -eq 1) { Write-Arch ' Skipped — still configured; ${client.promptName} may fail to reach it this session' DarkGray }
  else { Write-Arch ' Skipped — still configured; ${client.promptName} may fail to reach them this session' DarkGray }
}
...
function Get-ArchWaitPromptText {
  if ($ActiveRemotes.Count -eq 1) { return 'Disconnect ' + $ActiveRemotes[0].FailName + ' from ${client.promptName} now? (Y/n) ' }
  return 'Disconnect all ' + $ActiveRemotes.Count + ' unreachable resources from ${client.promptName} now? (Y/n) '
}
 ...
 * hooks the `<binary>` wrapper function into profile.ps1 of every PowerShell
 * edition present, inside an idempotent marker block. Relies on the setup
 * script's Say/Ok helpers being defined.
 */
export function buildWindowsStartupGuardInstallSection(
  ctx: StartupGuardContext,
  client: StartupGuardClient,
): string {
...
# Wrap \`${client.binary}\` in each PowerShell edition's all-hosts profile so the guard
...
foreach ($archProfilePath in $archProfiles) {
  ...
  if (Test-Path $archProfilePath) {
    ...
    foreach ($archLine in @(Get-Content -Path $archProfilePath)) {
      if ($archLine -eq ${psq(client.markerStart)}) { $archSkip = $true; continue }
      if ($archLine -eq ${psq(client.markerEnd)}) { $archSkip = $false; continue }
 ...
 * `<binary>` before the connect steps run it. `irm <url> | iex` executes in the
 * CURRENT PowerShell session — where the profile's `<binary>` wrapper is already
 * loaded — so removing `Function:<binary>` here is what stops the old guard from
 ...
 * {@link buildWindowsStartupGuardInstallSection}, which overwrites the guard
 ...
 */
export function buildWindowsStartupGuardUnshadowSection(
  client: StartupGuardClient,
): string {
  return `# A previous connect may have installed the ${client.binary} startup guard. irm|iex runs
# in the current session, where the profile's ${client.binary} wrapper is already loaded —
# drop it so the steps below reach the real ${client.binary}. Deliberately non-destructive:
...
Remove-Item Function:${client.binary} -ErrorAction SilentlyContinue`;
}
