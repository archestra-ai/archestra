...
async function resolveCapabilitiesForModel(
  modelId: string,
  provider: SupportedProvider,
): Promise<ModelCapabilities> {
  return getModelCapabilities(modelId, provider);
}
...
async function fetchAnthropicModels(apiKey: string): Promise<ModelInfo[]> {
  ...
  return Promise.all(
    data.data.map(async (model) => {
      const capabilities = await resolveCapabilitiesForModel(
        model.id,
        "anthropic",
      );
...
async function fetchOpenAiModels(apiKey: string): Promise<ModelInfo[]> {
  ...
  return Promise.all(filteredModels.map(mapOpenAiModelToModelInfo));
}
...
export async function mapOpenAiModelToModelInfo(
  ...
  const capabilities = await resolveCapabilitiesForModel(model.id, provider);
...
export async function fetchGeminiModels(apiKey: string): Promise<ModelInfo[]> {
  ...
  return Promise.all(
    chatModels.map(async (model) => {
      ...
      const capabilities = await resolveCapabilitiesForModel(modelId, "gemini");
...
async function fetchVllmModels(apiKey: string): Promise<ModelInfo[]> {
  ...
  return Promise.all(
    data.data.map(async (model) => {
      const capabilities = await resolveCapabilitiesForModel(model.id, "vllm");
