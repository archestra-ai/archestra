...
- Route files: per-entity folders — see `backend/CLAUDE.md` for the convention and the `virtual-api-key` canonical reference. Entities not yet migrated remain flat (`src/routes/<entity>.ts` + colocated `.test.ts`).
