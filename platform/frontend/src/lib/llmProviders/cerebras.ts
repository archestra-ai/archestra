...
// Define more precise types for Cerebras parts since the generated types use union discrimination
type CerebrasFunctionCallPart = {
  thought?: boolean;
...
type CerebrasFunctionResponsePart = {
  thought?: boolean;
  thoughtSignature?: string;
  functionResponse: {
    id?: string;
...
type CerebrasTextPart = {
  thought?: boolean;
  thoughtSignature?: string;
  text: string;
};

type CerebrasInlineDataPart = {
  thought?: boolean;
  thoughtSignature?: string;
  inlineData: {
    mimeType: string;
...
function hasFunctionResponse(
  part: unknown,
): part is CerebrasFunctionResponsePart {
  return (
    typeof part === "object" &&
    part !== null &&
...
function hasFunctionCall(part: unknown): part is CerebrasFunctionCallPart {
  return (
    typeof part === "object" &&
    part !== null &&
...
function hasText(part: unknown): part is CerebrasTextPart {
  return (
    typeof part === "object" &&
    part !== null &&
    "text" in part &&
    typeof (part as CerebrasTextPart).text === "string"
  );
}

function hasInlineData(part: unknown): part is CerebrasInlineDataPart {
  return (
    typeof part === "object" &&
    part !== null &&
    "inlineData" in part &&
...
function hasFileData(part: unknown): part is CerebrasFileDataPart {
  return (
    typeof part === "object" &&
    part !== null &&
    "fileData" in part &&
...
class CerebrasGenerateContentInteraction implements InteractionUtils {
  private request: archestraApiTypes.CerebrasGenerateContentRequest;
  private response: archestraApiTypes.CerebrasGenerateContentResponse;
  modelName: string;

  constructor(interaction: Interaction) {
    this.request =
      interaction.request as archestraApiTypes.CerebrasGenerateContentRequest;
    this.response =
      interaction.response as archestraApiTypes.CerebrasGenerateContentResponse;
  ...
  private mapToUiMessage(
    content:
      | archestraApiTypes.CerebrasGenerateContentRequest["contents"][number]
      | {
          role: "model";
          content: archestraApiTypes.CerebrasGenerateContentResponse["candidates"][number]["content"];
        },
    ...
    for (const part of contentParts) {
      if (hasText(part) && part.text) {
        ...
        parts.push({
          ...
          toolCallId: part.functionCall.id || `cerebras-${Date.now()}`,
              ...
              if (
                ...
                if (
                  ...
                  toolCallParts.push({
                    ...
                    toolCallId: functionCallId || `cerebras-${Date.now()}`,
...
export default CerebrasGenerateContentInteraction;
