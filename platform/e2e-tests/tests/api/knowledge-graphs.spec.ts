...
test.describe("Knowledge Graphs API", () => {
  ...
  test.describe("HMAC Internal Auth", () => {
    test("should reject internal sync endpoint without auth headers", async ({
      request,
      makeApiRequest,
      createKnowledgeGraph,
      createConnector,
      ...
      const connRes = await createConnector(
        request,
        kg.id,
        `E2E Conn HMAC ${uniqueSuffix}`,
      );
      const connector = await connRes.json();

      // Call internal endpoint without HMAC headers - should fail
      const response = await makeApiRequest({
        request,
        method: "post",
        urlSuffix: `/api/internal/connectors/${connector.id}/sync`,
        ignoreStatusCheck: true,
      });
    ...
    test("should reject internal sync endpoint with invalid signature", async ({
      request,
      makeApiRequest,
      createKnowledgeGraph,
      createConnector,
      ...
      const kgRes = await createKnowledgeGraph(
        request,
        `E2E KG HMAC Invalid ${uniqueSuffix}`,
      );
      const kg = await kgRes.json();
      ...
      const connRes = await createConnector(
        request,
        kg.id,
        `E2E Conn HMAC Invalid ${uniqueSuffix}`,
      );
      const connector = await connRes.json();
    ...
    test("should accept internal sync endpoint with valid HMAC signature", async ({
      request,
      makeApiRequest,
      createKnowledgeGraph,
      createConnector,
      ...
      const kgRes = await createKnowledgeGraph(
        request,
        `E2E KG HMAC Valid ${uniqueSuffix}`,
      );
      const kg = await kgRes.json();
      ...
      const connRes = await createConnector(
        request,
        kg.id,
        `E2E Conn HMAC Valid ${uniqueSuffix}`,
      );
      const connector = await connRes.json();

      // Generate valid HMAC signature using the auth secret from env
      const hmacSecret =
        process.env.ARCHESTRA_AUTH_SECRET ||
        "auth-secret-must-be-at-least-32-chars-long";
      const timestamp = Math.floor(Date.now() / 1000).toString();
      ...
      // The endpoint should authenticate successfully (2xx) or fail gracefully
      // because the actual sync may fail without a real KG backend, but auth should pass.
      // A 401 would mean auth failed, which is not expected with valid HMAC.
