const SERVICE_LOGO_ICONS: Record<
  string,
  {
    hex: string;
    path: string;
...
export function getServiceLogoIconSrc(icon?: string | null): string | null {
  ...
  const serviceLogo = SERVICE_LOGO_ICONS[slug];
  if (!serviceLogo) return null;

  return `data:image/svg+xml,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#${serviceLogo.hex}"><path d="${serviceLogo.path}"/></svg>`,
  )}`;
}
