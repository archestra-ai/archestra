/**
 * OpenRouter Interaction Utils
 *
 * OpenRouter uses OpenAI-compatible API format, so we reuse the OpenAI
 * interaction implementation with adjusted type imports.
 */
 ...
 * so we can use the same types.
 */
type OpenRouterRequest = archestraApiTypes.OpenAiChatCompletionRequest;
type OpenRouterResponse = archestraApiTypes.OpenAiChatCompletionResponse;
...
class OpenRouterChatCompletionInteraction implements InteractionUtils {
    ...
    constructor(interaction: Interaction) {
        this.request = interaction.request as OpenRouterRequest;
        this.response = interaction.response as OpenRouterResponse;
    ...
    private mapToUiMessage(
        message:
            | OpenRouterRequest["messages"][number]
            | OpenRouterResponse["choices"][number]["message"],
    ): PartialUIMessage {
        ...
        const roleMap: Record<
            OpenRouterRequest["messages"][number]["role"],
