...
const CONNECTOR_OPTIONS: {
  ...
  {
    type: "dropbox",
    label: CONNECTOR_TYPE_LABELS.dropbox,
    description: "Sync files and documents from Dropbox",
  },
];
...
export function CreateConnectorDialog({
  ...
  const handleSelectType = (type: ConnectorType) => {
    ...
    const defaultConfigs: Record<ConnectorType, Record<string, unknown>> = {
      ...
      dropbox: { type, recursive: true, folderPath: "" },
    };
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                              ...
                              : connectorType === "notion"
                                ...
                                : connectorType === "sharepoint"
                                  ...
                                  : connectorType === "gdrive"
                                    ...
                                    : connectorType === "dropbox"
                                      ? "sl.... (OAuth2 access token or app access token)"
                                      : needsEmail
                                        ? emailRequired
                                          ? "Your API token"
                      ...
                      {connectorType === "dropbox" && (
                        <p className="text-[0.8rem] text-muted-foreground">
                          Paste a Dropbox OAuth2 access token or app access
                          token with the <code>files.metadata.read</code> and{" "}
                          <code>files.content.read</code> scopes. Create one at
