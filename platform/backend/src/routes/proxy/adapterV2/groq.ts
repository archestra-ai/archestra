 ...
 *
 * Groq uses an OpenAI-compatible API at https://api.groq.ai/v1
 * This adapter reuses OpenAI's logic with Groq-specific configuration.
 *
 * @see https://inference-docs.groq.ai/
 */
  ...
  implements LLMRequestAdapter<GroqRequest, GroqMessages>
{
  ...
  convertToolResultContent(messages: GroqMessages): GroqMessages {
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to Groq format
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[GroqAdapter] Stripped browser tool results from messages",
        );
      }
    }
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[GroqAdapter] Stripped image blocks from tool result",
    );
  }
...
async function convertToolResultsToToon(
  ...
  // Use tiktoken tokenizer (same as OpenAI) for Groq
...
> = {
  ...
  async executeStream(
    ...
    const stream =
      await groqClient.chat.completions.create(groqRequest);
  ...
  extractErrorMessage(error: unknown): string {
    // OpenAI SDK error structure (same for Groq)
