...
| `entity_id` | `string` | Yes | The ID of the entity. UUIDs for agent / virtual_api_key, text IDs for organization / team / user. |
...
| `model` | `string[]` | No | Array of model names for token_cost limits. Use ["*"] as the sole element to cover every model (ALL_MODELS_SENTINEL). Mixing "*" with concrete model names is rejected. |
...
| `entity_id` | `string` | No | Optional filter by entity ID (UUID for agent/virtual_api_key, text for organization/team/user). |
