...
/**
 * Revoke credentials for an MCP server: stops pods for local servers and clears stored secrets without deleting the server record
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `mcpServerInstallation:update`: Modify installed MCP server configuration
 */
export const revokeCredentialsMcpServer = <ThrowOnError extends boolean = false>(options: Options<RevokeCredentialsMcpServerData, ThrowOnError>) => (options.client ?? client).post<RevokeCredentialsMcpServerResponses, RevokeCredentialsMcpServerErrors, ThrowOnError>({ url: '/api/mcp_server/{id}/revoke-credentials', ...options });
