 ...
 * - checks are paced to a ~0.5s minimum with a spinner; a REMOVED remote
 *   prompts immediately, an UNREACHABLE one gets the 15s capped-backoff
 *   retry ladder with jitter, status line at 3s, "hang tight" at 10s, and
 *   `s`/`d` hotkeys live throughout;
...
export function renderClaudeCodeStartupGuardPowerShell(
  ...
  const remoteEntries = resources
    .map(
      (r) =>
        `  @{ Label = ${psq(r.label)}; Url = ${psq(r.url)}; Kind = ${psq(r.kind)}; HealthUrl = ${psq(r.healthUrl ?? "")} }`,
    )
    .join("\n");
...
# platform's answer is authoritative, so the guard prompts immediately.
...
# Probe one remote: 'ok' | 'down' | 'missing'. With a health URL, the platform
# answers ok/missing authoritatively; an older backend 404s the then-unknown
# route (a response with no "missing" marker), degrading to reachability-only
# — version skew can never read as a deleted remote. Without a health URL, any
# HTTP response within the timeout counts as ok (401 included). Works on 5.1
# (WebException) and 7+ (HttpResponseException): both expose .Response only
# when a server actually answered.
function Test-ArchRemote($r) {
  if ($r.HealthUrl) {
    try {
      $resp = Invoke-WebRequest -Uri $r.HealthUrl -Method Get -TimeoutSec 3 -UseBasicParsing -ErrorAction Stop
      if ([string]$resp.Content -match '"status":"missing"') { return 'missing' }
      return 'ok'
    } catch {
      if ($_.Exception.PSObject.Properties['Response'] -and $_.Exception.Response) { return 'ok' }
      return 'down'
    }
  }
  try {
    $null = Invoke-WebRequest -Uri $r.Url -Method Get -TimeoutSec 3 -UseBasicParsing -ErrorAction Stop
    return 'ok'
  } catch {
    if ($_.Exception.PSObject.Properties['Response'] -and $_.Exception.Response) { return 'ok' }
    return 'down'
...
if (-not $Interactive) {
  foreach ($r in $Remotes) {
    switch (Test-ArchRemote $r) {
      'down' { [Console]::Error.WriteLine('archestra: ' + $r.Label + ' is unreachable — claude may fail to reach it. Disconnect it from the ' + $AppName + ' /connection page, or run claude interactively to be offered a disconnect.') }
      'missing' { [Console]::Error.WriteLine('archestra: ' + $r.Label + ' was removed on ' + $AppName + ' but claude is still configured to use it — run claude interactively to disconnect it, or use the /connection page.') }
    }
  }
  exit 0
}
...
function Show-ArchMissingPrompt([string]$Label, [string]$Kind) {
  Clear-ArchLine
  Write-Arch ('✖ ' + $Label + ' was removed on ' + $AppName) Red -NoNewline
  Write-Arch ' (the platform reports it no longer exists)' DarkGray
  Write-Arch '  claude is still configured to use it and will keep failing to connect.' DarkGray
  Write-Host '  [d] disconnect it from Claude Code (recommended)   [s] keep it (default)'
  $choice = ''
  try { $choice = [string][Console]::ReadKey($true).KeyChar } catch { $choice = 's' }
  if ($choice -eq 'd' -or $choice -eq 'D') { Disconnect-ArchRemote $Kind $Label }
  else { Show-ArchSkipped $Label }
}

function Test-ArchRemoteWithRetry($r) {
  Show-ArchSpin $r.Label ''
  $state = Test-ArchRemote $r
  ...
  if ($state -eq 'ok') { Show-ArchOk $r.Label; return }
  ...
  while ($true) {
    ...
    if ($elapsed -ge $HangTightAfterSeconds) {
      Show-ArchSpin $r.Label ('— trying to connect... ' + $elapsed + 's, few more seconds, hang tight...  [s] skip  [d] disconnect')
    } elseif ($elapsed -ge $NoticeAfterSeconds) {
      Show-ArchSpin $r.Label ('— trying to connect... ' + $elapsed + 's  [s] skip  [d] disconnect')
    }
  }
  Clear-ArchLine
...
/**
 * Same remotes, same pre-loader order, same health URLs as the bash renderer
 * (both are fed by buildClaudeCodeStartupGuardContext). Skills marketplace is
 * reachability-only — a revoked share link never blocks a claude launch.
 */
