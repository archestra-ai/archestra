...
3. \`archestra__edit_app\` — build the app up with the \`sections\` argument: pass css/body/javascript (and title) by value, replacing a whole section or patching it in place with \`{ edits: [...] }\`; omitted sections are unchanged, and you never send or match HTML boilerplate. Read a section back first with \`archestra__read_app\` (format: "sections") only when you need its current source. The raw \`edits\`/\`replacementHtml\` modes are the escape hatch for a genuinely custom document (a full replacement that drops the managed sections turns the app raw). Before writing code that parses an assigned tool's result, call \`archestra__preview_app_tool\` to see its real output shape.
