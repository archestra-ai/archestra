    ...
    const [sessionsData, [{ total }]] = await Promise.all([
      db
        .select({
          ...
          toonAddonUnavailableCount: sql<number>`COUNT(*) FILTER (WHERE ${schema.interactionsTable.toonSkipReason} = 'addon_unavailable')`,
    ...
    const sessions = sessionsData.map((s) => {
      ...
      return {
        ...
        toonSkipReasonCounts: {
          ...
          addonUnavailable: Number(s.toonAddonUnavailableCount) || 0,
        },
