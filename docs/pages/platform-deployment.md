...
- `ARCHESTRA_OPENAPPA_OTEL_ENABLED`: defaults to `false`. Exports embedded APPA traces, logs, and metrics when APPA initializes. Uses `ARCHESTRA_OTEL_EXPORTER_OTLP_ENDPOINT` and its authentication settings. The collector must accept all three signals. Raw prompts, tool arguments, and results are excluded. Names, identifiers, and approved report messages can contain sensitive information. See [OpenAPPA telemetry](/docs/platform-observability#openappa-telemetry).
