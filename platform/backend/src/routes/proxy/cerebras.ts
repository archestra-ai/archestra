 ...
 * V2 routes are located at: ./routesv2/cerebras.ts
 *
...
import OpenAIProvider from "cerebras";
import { z } from "zod";
...
import { MockOpenAIClient } from "./mock-cerebras-client";
...
const openAiProxyRoutes: FastifyPluginAsyncZod = async (fastify) => {
   ...
   * - /v1/cerebras/:agentId/* -> config.llm.cerebras.baseUrl/* (agentId stripped if UUID)
   *  - /v1/cerebras/* -> config.llm.cerebras.baseUrl/* (direct proxy)
   *
        ...
        fastify.log.info(
          {
            ...
            upstream: config.llm.cerebras.baseUrl,
            finalProxyUrl: `${config.llm.cerebras.baseUrl}/v1${remainingPath}`,
          },
        ...
        fastify.log.info(
          {
            ...
            url: request.url,
            upstream: config.llm.cerebras.baseUrl,
            finalProxyUrl: `${config.llm.cerebras.baseUrl}/v1${pathAfterPrefix}`,
          },
  ...
  const handleChatCompletion = async (
    ...
    const openAiClient = config.benchmark.mockMode
      ...
      : new OpenAIProvider({
          ...
          fetch: getObservableFetch("cerebras", resolvedAgent, externalAgentId),
        });
    ...
    try {
      ...
      const commonMessages = utils.adapters.cerebras.toCommonFormat(messages);
      logger.debug(
      ...
      let filteredMessages = utils.adapters.cerebras.applyUpdates(
        messages,
        toolResultUpdates,
      );
      ...
      if (shouldApplyToonCompression) {
        const { messages: convertedMessages, stats } =
          await utils.adapters.cerebras.convertToolResultsToToon(
            filteredMessages,
            model,
          );
      ...
      fastify.log.info(
        ...
        "cerebras proxy routes: handle chat completions: tool results compression completed",
      );
      ...
      if (stream) {
        ...
        try {
          for await (const chunk of streamingResponse) {
            ...
            if (chunk.usage) {
              tokenUsage = utils.adapters.cerebras.getUsageTokens(chunk.usage);
            }
          ...
          if (tokenUsage) {
            ...
            fastify.log.info(
              ...
              "cerebras proxy routes: handle chat completions: costs",
            );
          } else {
            fastify.log.warn(
        ...
        const tokenUsage = response.usage
          ? utils.adapters.cerebras.getUsageTokens(response.usage)
