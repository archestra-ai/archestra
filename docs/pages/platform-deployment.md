...
- **`ARCHESTRA_ORCHESTRATOR_MCP_K8S_SERVICE_ACCOUNT_NAME`** - Kubernetes service account name for MCP server pods that need K8s API access.

  - Default: `archestra-archestra-platform-mcp-k8s-operator`. 
  - When deploying via official Helm chart, this is matches name of the SeviceAccount created there. ServiceAccout name is derived as ``` {{ include "archestra-platform.fullname" . }}-mcp-k8s-operator```
  - Customize if you use a custom Helm release name or create a ServiceAccount manually.
