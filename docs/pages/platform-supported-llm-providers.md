...
5. For a **single-tenant** registration, also set `ARCHESTRA_MICROSOFT_COPILOT_TENANT_ID` to your tenant ID. The default (`organizations`) only works for multi-tenant registrations.
...
To connect, use the **Sign in with Microsoft** button when adding a Microsoft Copilot key. It runs Entra's OAuth device flow — you approve a one-time code on Microsoft's device sign-in page, and Archestra stores the resulting refresh token.
...
- **Per-user only**: keys are **personal scope only**, same as GitHub Copilot. Each user connects their own Microsoft account; an inline "Connect Microsoft Copilot" card appears in chat when a key is missing. Every request runs as the signed-in user — Copilot only sees data that user can already access.
