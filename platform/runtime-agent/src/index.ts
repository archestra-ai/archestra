...
/**
 * The agent loop that runs inside an Agent Runtime workload.
 *
      ...
      if (messages.length === 0 || messages.at(-1)?.role === "assistant") {
        ...
        if (incoming.length === 0) {
          if (config.idleTimeoutMs !== null) {
            write("[runtime] no further direction — session complete, exiting");
          }
          break;
        }
...
function renderHeader(config: RuntimeAgentConfig): void {
  if (config.banner) {
    process.stdout.write(`${ANSI.accent}${config.banner}${ANSI.reset}\n\n`);
  }
  write(`${ANSI.bold}${config.agentName}${ANSI.reset}`);
  write(
    `${ANSI.dim}${config.model} · isolated run · ${
      config.runMode === "interactive" ? "interactive" : "one-shot"
    }${ANSI.reset}`,
  );
...
function createModel(config: RuntimeAgentConfig) {
  const headers = runHeaders(config.taskId);
 ...
 */
async function connectGateway(config: RuntimeAgentConfig): Promise<Client> {
    ...
    new URL(config.gatewayUrl),
    {
      requestInit: {
        headers: {
          ...
          ...runHeaders(config.taskId),
        },
      },
    },
  );
  const client = new Client({
    name: "archestra-runtime-agent",
    version: "0.1.0",
  });
  try {
...
function runHeaders(taskId: string): Record<string, string> {
  return {
