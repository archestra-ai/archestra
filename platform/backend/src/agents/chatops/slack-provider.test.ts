...
describe("SlackProvider.parseWebhookNotification — background execution reaction", () => {
  const BOT = "UBOT123";
  const REACTOR = "U_REACTOR";
  const AUTHOR = "U_AUTHOR";
  const CHANNEL = "C_TASKS";
  const MESSAGE_TS = "8888888888.000001";

  function createBackgroundReactionProvider(message: {
    text?: string;
    user?: string;
    bot_id?: string;
    subtype?: string;
    ...
    const history = vi.fn().mockResolvedValue({
      messages: [{ ts: MESSAGE_TS, user: AUTHOR, ...message }],
    });
    // biome-ignore lint/suspicious/noExplicitAny: test-only — inject client mock
    (provider as any).client = {
  ...
  function crabReactionPayload(overrides: Record<string, unknown> = {}) {
    return makeEventPayload(
      {},
      {
        type: "reaction_added",
  ...
  test("🦀 delegates the reacted-to message as the reacting user", async () => {
    ...
    expect(result).toMatchObject({
      ...
      text: ":crab: Add model fallback support",
      rawText: ":crab: Add model fallback support",
      isThreadReply: false,
      metadata: {
