...
- `ARCHESTRA_OPENAPPA_OFFER_SIGNING_SECRET`: HMAC secret for signed offer tokens on `get_remedy_plans` / `execute_remedy_plan` and single-use native-question receipts. The proxy attaches a flattened JWS JSON serialization (RFC 7515 §7.2.2) with an unencoded payload (RFC 7797): `protected`, `payload`, and `signature`. This provides integrity verification (JWS), not encryption (JWE). The `protected` header specifies `alg: HS256` and `kid: default`. Unknown algorithms fail closed. Remedy arguments (`offer_id`, `plan`) and the execution receipt stay outside the JWS. Optional. Helm deployments generate and preserve an `offer-signing-secret` key across upgrades. Other deployments derive a key from the session authentication secret. Set this variable (minimum 32 characters) to configure an explicit key or rotate keys independently. Every backend replica must resolve to the same value.
