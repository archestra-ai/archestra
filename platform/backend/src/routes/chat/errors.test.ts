...
describe("mapProviderError", () => {
  describe("AI_APICallError with deeply nested invalid API key error (Gemini 400)", () => {
    ...
    it("should map to authentication error for deeply nested API key message", () => {
      const result = mapProviderError(geminiDeeplyNestedError);
    ...
    it("should detect gemini as the provider", () => {
      const result = mapProviderError(geminiDeeplyNestedError);
  ...
  describe("AI_APICallError with invalid API key (Gemini 400)", () => {
    ...
    it("should map to authentication error for invalid API key message", () => {
      const result = mapProviderError(geminiInvalidApiKeyError);
  ...
  describe("AI_RetryError with nested AI_APICallError (Gemini)", () => {
    ...
    it("should map to server_error for 500 status code", () => {
      const result = mapProviderError(geminiRetryError);
  ...
  describe("status code mapping", () => {
    it("should map 400 to InvalidRequest", () => {
      ...
      const result = mapProviderError(error);
  ...
  describe("provider detection", () => {
    it("should detect anthropic provider", () => {
      const error = {
        message: "Error from anthropic API",
        statusCode: 500,
      };
      const result = mapProviderError(error);
    ...
    it("should detect anthropic from claude reference", () => {
      const error = {
        message: "Claude model error",
        statusCode: 500,
      };
      const result = mapProviderError(error);
    ...
    it("should detect openai from gpt reference", () => {
      const error = {
        message: "gpt-4 model error",
        statusCode: 500,
      };
      const result = mapProviderError(error);
    ...
    it("should detect gemini from google reference", () => {
      const error = {
        message: "google generativelanguage error",
        statusCode: 500,
      };
      const result = mapProviderError(error);
  ...
  describe("fallback behavior", () => {
    ...
    it("should handle string errors", () => {
      const result = mapProviderError("Simple string error");
