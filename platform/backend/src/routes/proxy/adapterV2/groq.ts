/**
 * Groq Adapter for LLM Proxy
 *
 * Groq is fully OpenAI-compatible, so this adapter reuses OpenAI's logic
 * with modified provider identification and base URL.
 */
  ...
  implements LLMStreamAdapter<GroqStreamChunk, GroqResponse>
{
  ...
  processChunk(chunk: GroqStreamChunk): ChunkProcessingResult {
    ...
    // Handle usage - Groq may send usage in a final chunk
    if (chunk.usage) {
      this.state.usage = {
...
async function convertToolResultsToToon(
  ...
  // Use tiktoken for Groq (OpenAI-compatible)
...
> = {
  ...
  extractApiKey(headers: GroqHeaders): string | undefined {
    // Groq uses the same Authorization header format as OpenAI
    // Strip "Bearer " prefix if present, since the OpenAI SDK will add it back
  ...
  createClient(
    ...
    // Groq uses the OpenAI SDK with a different base URL
  ...
  extractErrorMessage(error: unknown): string {
    // Groq uses the same error structure as OpenAI
