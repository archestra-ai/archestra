...
describe("MCP OAuth clients — team-scope RBAC", () => {
  ...
  beforeEach(async ({ makeOrganization, makeAgent }) => {
    ...
    gatewayId = (await makeAgent({ organizationId, agentType: "mcp_gateway" }))
      .id;
    ...
    const { default: routes } = await import("./mcp-oauth-clients");
  ...
  function list() {
    return app.inject({ method: "GET", url: "/api/mcp-oauth-clients" });
  }
  ...
  test("legacy row without scope/authorId metadata lists as org-scoped and is visible to everyone", async ({
    ...
    // scope/authorId (and no grantType/allowedGatewayIds).
    await db.insert(schema.oauthClientsTable).values({
      id: crypto.randomUUID(),
      clientId: `mcp_oauth_legacy_${crypto.randomUUID().slice(0, 8)}`,
      name: "legacy-client",
      redirectUris: [],
      metadata: { type: MCP_OAUTH_CLIENT_METADATA_TYPE, organizationId },
      createdAt: new Date(),
      updatedAt: new Date(),
    });
