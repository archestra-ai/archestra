...
The sweep leaves a record in place when it cannot trace that record to an organization, and logs a warning instead. An MCP server install belongs to an organization through its team, its catalog entry, or its owner's membership. The owner only settles it when they belong to a single organization — otherwise the sweep skips the install rather than pick one.
