 ...
 *
 * Allowlist:
 *   Packages with Unknown licenses can be manually verified and added to the
 *   VERIFIED_LICENSES constant at the top of this script. Include:
...
// Manually verified licenses for packages without proper license metadata
// Format: "package-name": { license: "License-Type", source: "verification URL" }
const VERIFIED_LICENSES = {
  "@instantdb/core": {
    license: "Apache-2.0",
    source: "https://github.com/instantdb/instant/blob/main/LICENSE"
  },
  "@instantdb/react": {
    license: "Apache-2.0",
    source: "https://github.com/instantdb/instant/blob/main/LICENSE"
  },
  "@instantdb/react-common": {
    license: "Apache-2.0",
    source: "https://github.com/instantdb/instant/blob/main/LICENSE"
  },
  "@instantdb/version": {
...
try {
      ...
      try {
        ...
        if (pkg.workspaces) {
          ...
          for (const workspace of workspaces) {
            const workspacePath = path.join(dir, workspace.replace(/\/\*$/, ""));
            if (fs.existsSync(workspacePath)) {
              const subdirs = fs.readdirSync(workspacePath, { withFileTypes: true });
              for (const subdir of subdirs) {
                if (subdir.isDirectory()) {
                  results.push(...findNodeModules(path.join(workspacePath, subdir.name), depth + 1));
                }
              }
            }
          }
      ...
      if (entry.name.startsWith("@")) {
        ...
        for (const scopedEntry of scopedEntries) {
          if (scopedEntry.isDirectory()) {
            ...
            const pkgPath = path.join(scopePath, scopedEntry.name, "package.json");
      ...
      if (fs.existsSync(pkgPath)) {
        ...
        for (const pkgEntry of pkgEntries) {
          if (pkgEntry.name.startsWith("@")) {
            ...
            const pkgJsonPath = path.join(pkgPath, pkgEntry.name, "package.json");
            readPackageJson(pkgName, pkgJsonPath);
          }
        }
      }
      ...
      https.get(url, { timeout: 3000 }, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            const license = pkg.license || pkg.licenses?.[0]?.type || pkg.licenses;
  ...
  function detectLicenseFromFile(content) {
    ...
    if (contentUpper.includes("GNU AFFERO GENERAL PUBLIC LICENSE")) return "AGPL-3.0";
    if (contentUpper.includes("MOZILLA PUBLIC LICENSE")) return "MPL-2.0";
    if (contentUpper.includes("UNLICENSE") || contentUpper.includes("PUBLIC DOMAIN")) return "Unlicense";
    ...
    if (contentUpper.includes("PYTHON SOFTWARE FOUNDATION")) return "Python-2.0";
  ...
  function readPackageJson(name, pkgPath) {
    ...
    try {
      ...
      if (!license || license === "Unknown" || license === "UNKNOWN" || license === "SEE LICENSE IN LICENSE") {
        const pkgDir = path.dirname(pkgPath);
        const licenseFiles = ["LICENSE", "LICENCE", "LICENSE.md", "LICENSE.txt", "COPYING", "LICENSE-MIT", "LICENSE-APACHE"];
      ...
      licenseMap.set(name, {
        version,
        license: typeof license === "object" ? JSON.stringify(license) : license
      });
    } catch (e) {
  ...
  console.log(`Scanning ${nodeModulesDirs.length} node_modules directories...`);
  ...
  for (const [name, info] of licenseMap.entries()) {
    if (info.license === "Unknown" || info.license === "UNKNOWN" || info.license === "SEE LICENSE IN LICENSE") {
      unknownPackages.push(name);
    }
  }

  if (unknownPackages.length > 0) {
    console.log(`Checking npm registry for ${unknownPackages.length} unknown licenses...`);
    const promises = unknownPackages.map(async (name) => {
      const npmLicense = await fetchLicenseFromNpm(name);
      if (npmLicense && npmLicense !== "Unknown") {
        const info = licenseMap.get(name);
        info.license = npmLicense;
        licenseMap.set(name, info);
      }
    });
  ...
  function categorizeLicense(license) {
    ...
    if (/GPL/i.test(license) && !/LGPL/i.test(license) && !/AGPL/i.test(license)) return "GPL";
  ...
  if (ciMode) {
    ...
    for (const [name, { version, license }] of licenseMap.entries()) {
      ...
      if (category === "Unknown" && VERIFIED_LICENSES[name]) {
        ...
        verifiedPackages.push({ name, version, license: verified.license, source: verified.source });
      ...
      if (category === "GPL" || category === "AGPL") {
        problematicPackages.push({ name, version, license: effectiveLicense, category });
      }

      // Multi-license with AND containing GPL is problematic
      if (category.startsWith("Multi:") && /GPL/i.test(effectiveLicense) && !/LGPL/i.test(effectiveLicense)) {
        problematicPackages.push({ name, version, license: effectiveLicense, category: "GPL (multi)" });
      }
    ...
    console.error(`❌ Found ${problematicPackages.length} problematic dependencies:\n`);
    ...
    console.error("  - For Unknown: Verify license and add to VERIFIED_LICENSES in script");
  ...
  for (const [name, { version, license }] of licenseMap.entries()) {
    ...
    if (filterType) {
      ...
      if (filterUpper === "GPL" && !["GPL", "LGPL", "AGPL"].includes(baseCategory)) continue;
      if (filterUpper === "MIT" && baseCategory !== "MIT") continue;
      if (filterUpper === "APACHE" && !baseCategory.includes("Apache")) continue;
      if (filterUpper === "BSD" && baseCategory !== "BSD") continue;
      if (filterUpper === "ISC" && baseCategory !== "ISC") continue;
      if (filterUpper === "OTHER" && ["GPL", "LGPL", "AGPL", "MIT", "Apache", "BSD", "ISC", "CC0", "Unlicense", "0BSD", "MPL"].includes(baseCategory)) continue;
    }
  ...
  function getRestrictiveness(category) {
    const baseCategory = category.replace(" (verified)", "").replace(" (dual)", "");

    // Public domain (most permissive)
    if (baseCategory === "Unlicense" || baseCategory === "CC0" || baseCategory === "WTFPL") return 0;
  ...
  if (hasVerified) {
    console.log("Note: (verified) = License manually verified from external source (see VERIFIED_LICENSES in script)\n");
  }

  for (const { name, version, license, category } of results) {
    const tag = `[${category}]`;
    console.log(`${tag.padEnd(25)} ${name}@${version} - ${license}`);
  }
