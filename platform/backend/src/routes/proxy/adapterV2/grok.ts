/**
 * Grok LLM Proxy Adapter - OpenAI-compatible
 *
 * Grok uses an OpenAI-compatible API at https://api.grok.ai/v1
 * This adapter reuses OpenAI's logic with Grok-specific configuration.
 *
...
type GrokToolResultTextBlock = {
  type: "text";
  text: string;
};

type GrokToolResultContentBlock =
  | GrokToolResultImageBlock
  | GrokToolResultTextBlock;

type GrokToolResultContent = string | GrokToolResultContentBlock[];
  ...
  implements LLMRequestAdapter<GrokRequest, GrokMessages>
{
  ...
  convertToolResultContent(messages: GrokMessages): GrokMessages {
    ...
    for (const message of messages) {
      if (message.role === "tool") {
        ...
        logger.info(
          ...
          "[GrokAdapter] Analyzing tool result content",
        );
          ...
          for (const [idx, item] of message.content.entries()) {
            if (typeof item === "object" && item !== null) {
              ...
              logger.info(
                ...
                "[GrokAdapter] Tool result array item",
              );
            }
          }
        }
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to Grok format
      const convertedContent = convertMcpImageBlocksToGrok(message.content);
      if (!convertedContent) {
        return message;
      }
    ...
    if (toolMessagesWithImages > 0 || strippedImageCount > 0) {
      logger.info(
        ...
        "[GrokAdapter] Processed tool messages with image content",
      );
    }
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[GrokAdapter] Stripped browser tool results from messages",
        );
      }
    }
...
function convertMcpImageBlocksToGrok(
  content: unknown,
): GrokToolResultContent | null {
  ...
  const grokContent: GrokToolResultContentBlock[] = [];
    ...
    if (isMcpImageBlock(item)) {
      ...
      if (shouldStripImage) {
        logger.info(
          ...
          "[GrokAdapter] Stripping MCP image block due to size limit",
        );
        grokContent.push({
          type: "text",
      ...
      logger.info(
        ...
        "[GrokAdapter] Converting MCP image block to Grok format",
      );
  ...
  logger.info(
    {
      totalBlocks: grokContent.length,
      imageBlocks: grokContent.filter((b) => b.type === "image_url").length,
      textBlocks: grokContent.filter((b) => b.type === "text").length,
    },
    "[GrokAdapter] Converted MCP content to Grok format",
  );

  return grokContent.length > 0 ? grokContent : null;
}
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[GrokAdapter] Stripped image blocks from tool result",
    );
  }
...
async function convertToolResultsToToon(
  ...
  // Use tiktoken tokenizer (same as OpenAI) for Grok
...
> = {
  ...
  createClient(
    ...
    // Use OpenAI SDK with Grok base URL (OpenAI-compatible API)
  ...
  async execute(
    ...
    const grokClient = client as OpenAIProvider;
    const grokRequest = {
      ...request,
      stream: false,
  ...
  async executeStream(
    ...
    const stream =
      await grokClient.chat.completions.create(grokRequest);

    return {
      [Symbol.asyncIterator]: async function* () {
        for await (const chunk of stream) {
          yield chunk as GrokStreamChunk;
        }
      },
    };
  },
  ...
  extractErrorMessage(error: unknown): string {
    // OpenAI SDK error structure (same for Grok)
