 ...
 * deployments and custom domains — no localhost loopback required. Two explicit
 * steps: fetch the code (button or autoStart), then open the ChatGPT page
 * (button).
 */
export function OpenaiCodexSignIn({
  ...
  // ChatGPT tab here — that is the separate, explicit open step below, kept
  ...
  // Step 2: open the ChatGPT verification page. Synchronous with the click and
  // opened with noopener so the new tab can't reach back through window.opener.
  const openVerification = (deviceFlow: OpenaiCodexDeviceStart) => {
