    ...
    try {
      ...
      if (attempt < MAX_RETRIES) {
        ...
        logger.warn(
          ...
          "[KnowledgeBase] Retryable HTTP error, will retry",
        );
        await sleep(delay);
        continue;
      }
      ...
      if (isRetryableError(error) && attempt < MAX_RETRIES) {
        ...
        logger.warn(
          ...
          "[KnowledgeBase] Transient error, will retry",
        );
        await sleep(delay);
        continue;
      }
...
/**
 * LightRAG Knowledge Base Provider
 *
 * Integrates with a LightRAG server to provide document ingestion
 * and knowledge base querying capabilities.
 */
export class LightRAGProvider implements KnowledgeBaseProvider {
  ...
  async initialize(): Promise<void> {
    ...
    logger.info(
      ...
      "[KnowledgeBase] LightRAG provider initialized",
    );
  }
  ...
  async cleanup(): Promise<void> {
    logger.info("[KnowledgeBase] LightRAG provider cleaned up");
  }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          ...
          "[KnowledgeBase] Failed to insert document into LightRAG",
        );
        return {
          documentId: "",
          status: "failed",
      ...
      logger.info(
        ...
        "[KnowledgeBase] Document inserted into LightRAG",
      );
      ...
      logger.error(
        ...
        "[KnowledgeBase] Error inserting document into LightRAG",
      );
      return {
        documentId: "",
        status: "failed",
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          ...
          "[KnowledgeBase] Failed to query LightRAG",
        );
        return {
          answer: "",
          error: `LightRAG API error: ${response.status} - ${errorText}`,
      ...
      logger.error(
        ...
        "[KnowledgeBase] Error querying LightRAG",
      );
      return {
        answer: "",
        error: errorMessage,
