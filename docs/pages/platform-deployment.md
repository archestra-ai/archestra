...
With the OpenAPPA flag disabled, existing Tool Guardrails run unchanged. The native runtime and its MCP tools remain inactive. When enabled, OpenAPPA replaces proxy tool-call and tool-result checks. Errors fail closed. A blocked call returns as a `get_remedy_plans` notice tool call. The model inspects the ruling, chooses a remedy with `execute_remedy_plan`, and retries. Tool-enabled requests must declare both APPA tools. Interactive human approval is not connected; affected calls remain blocked. See [the integration setup](https://github.com/archestra-ai/archestra/blob/main/platform/archestra-rs/openappa-rs/README.md) for current limitations.

Notice restoration and turn accounting support Anthropic Messages, OpenAI Responses, and OpenAI Chat Completions. Bedrock InvokeModel uses the Anthropic restoration. Other protocols still receive call and result checks, but notices remain in history.

Remedy routing and execution receipts are stored in PostgreSQL. Any backend replica using that database can resolve an offer after a restart or routing change. The runtime validates the offer and control-call vouch before execution.

The proxy adds a typed execution frame with the provider's real tool call ID. Ordinary MCP clients return it without changes. The same logical ID and arguments return the recorded result. Changed arguments under that ID are refused. A new ID for a spent offer returns terminal feedback. Direct calls need a stable logical ID because JSON-RPC request IDs can be reused.

Ordinary sessions are shared by authorized participants in one organization. External client sessions are scoped to the authenticated credential. A personal offer requires its originating user. An organization-scoped offer allows an organization caller. Remedy receipts bind the actual spender.

Without a session signal, requests share a fallback root per credential and agent. Uncredentialed loopback is trusted as platform traffic. A frontend forwarding `/v1` over loopback extends that trust boundary to its callers.
