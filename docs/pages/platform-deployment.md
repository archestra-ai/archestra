...
- **`ARCHESTRA_SANDBOX_RS_LOG`** - Log-level filter for the native skill-sandbox runtime (the Rust/napi layer behind Agent Skill execution).
  - Default: `info`
  - Accepts a [`tracing` env filter](https://docs.rs/tracing-subscriber/latest/tracing_subscriber/filter/struct.EnvFilter.html) directive (e.g. `warn`, `info`, `sandbox_core=debug`)
  - Affects native log verbosity only; trace spans are always exported and reuse `ARCHESTRA_OTEL_EXPORTER_OTLP_ENDPOINT`
