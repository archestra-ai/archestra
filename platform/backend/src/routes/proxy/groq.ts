...
import { MockGroqClient } from "./mock-groq-client";
...
const openAiProxyRoutes: FastifyPluginAsyncZod = async (fastify) => {
  ...
  const handleChatCompletion = async (
    ...
    const openAiClient = config.benchmark.mockMode
      ? (new MockGroqClient() as unknown as GroqProvider)
      : new GroqProvider({
          apiKey: openAiApiKey,
    ...
    try {
      ...
      const commonMessages = utils.adapters.groq.toCommonFormat(messages);
      logger.debug(
      ...
      // Apply updates back to Groq messages
      logger.debug(
        {
      ...
      let filteredMessages = utils.adapters.groq.applyUpdates(
        messages,
        toolResultUpdates,
      );
      ...
      if (shouldApplyToonCompression) {
        const { messages: convertedMessages, stats } =
          await utils.adapters.groq.convertToolResultsToToon(
            filteredMessages,
            model,
          );
      ...
      if (stream) {
        ...
        const accumulatedToolCalls: GroqProvider.Chat.Completions.ChatCompletionMessageFunctionToolCall[] =
          [];
        const chunks: GroqProvider.Chat.Completions.ChatCompletionChunk[] =
          [];
        let tokenUsage: { input?: number; output?: number } | undefined;

        // Variables for interaction recording (accessible in finally block)
        let assistantMessage:
          | GroqProvider.Chat.Completions.ChatCompletionMessage
          | undefined;
        ...
        try {
          for await (const chunk of streamingResponse) {
            ...
            if (chunk.usage) {
              tokenUsage = utils.adapters.groq.getUsageTokens(chunk.usage);
            }
          ...
          if (accumulatedToolCalls.length > 0) {
            ...
            if (toolInvocationRefusal) {
                ...
                choices: [
                  {
                    index: 0,
                    delta:
                      assistantMessage as GroqProvider.Chat.Completions.ChatCompletionChunk.Choice.Delta,
              ...
              // We must match Groq's actual streaming format: send separate chunks for id, name, and arguments
        ...
        logger.debug(
          ...
          "[GroqProxy] Non-streaming response received, checking tool invocation policies",
        );
        ...
        if (toolInvocationRefusal) {
          ...
          logger.debug(
            { toolCallCount: assistantMessage.tool_calls?.length || 0 },
            "[GroqProxy] Tool invocation blocked by policy",
          );
        ...
        const tokenUsage = response.usage
          ? utils.adapters.groq.getUsageTokens(response.usage)
