...
async function handleMcpPostRequest(
  ...
  try {
    ...
    fastify.log.debug({ profileId }, "Connecting server to transport");
    await server.connect(transport);
    fastify.log.debug({ profileId }, "Server connected to transport");

    fastify.log.debug({ profileId }, "Calling transport.handleRequest");
    ...
    fastify.log.debug({ profileId }, "Transport.handleRequest completed");
    ...
    if (isInitialize) {
      try {
        ...
        fastify.log.debug({ profileId }, "Saved initialize request");
      } catch (dbError) {
        fastify.log.error(
          { err: dbError },
    ...
    fastify.log.debug({ profileId }, "Request handled successfully");
  } catch (error) {
    fastify.log.error(
      {
        error,
