...
const registry = defineArchestraTools([
  defineArchestraTool({
    ...
    title: "Run Tool",
    description: `Dispatch to any tool available to this agent, including built-in platform tools, agent delegation tools ('agent-<id>'), or third-party MCP tools exposed through the MCP Gateway (e.g. 'context7__resolve-library-id'). When the agent allows dynamic tool access, a tool the user can access but the agent does not have runs directly without being assigned to the agent; the MCP server's connection policy decides which credential the call uses. Target-tool RBAC, invocation policies, argument validation, and output validation all still apply. The app-authoring tools are reached this way too: when asked to make, build, or create an interactive app, start with 'scaffold_app' through this tool instead of writing app code in a reply.`,
