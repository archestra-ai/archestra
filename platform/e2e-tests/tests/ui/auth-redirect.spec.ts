...
test.describe("Authentication redirect flows", { tag: ["@firefox", "@webkit"] }, () => {
  test("sign-out works and redirects to sign-in page", async ({
    ...
    // Should be redirected to sign-out page, then eventually to sign-in
    await adminPage.waitForURL(/\/auth\/sign-out|\/auth\/sign-in/);

    // After sign-out completes, should show sign-in form
    await expect(
      adminPage.getByRole("heading", { name: /sign in/i }),
  ...
  test("redirectTo parameter is validated (rejects malicious URLs)", async ({
    ...
    try {
      ...
      // Should be redirected to home page (not the malicious URL)
      await page.waitForURL(`${UI_BASE_URL}/**`, { timeout: 15000 });

      // Should NOT be on the malicious URL
      expect(page.url()).not.toContain("evil.com");

      // Should be on a valid app page (home or chat)
      expect(
        page.url().startsWith(UI_BASE_URL),
      ).toBe(true);
    } finally {
      await context.close();
    }
