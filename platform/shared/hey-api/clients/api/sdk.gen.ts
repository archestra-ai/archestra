...
/**
 * Remove the calling user's mute on one MCP connection alert, bringing it back into their own view. 404 when the caller has no mute for that alert.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
...
/**
 * Mute one alert on an MCP connection for the calling user only — it stays visible to everyone else. Only `needs-reauth` is mutable, and only while the connection is actually reporting it: the mute is pinned to that refresh failure and stops applying the moment a fresh one is recorded. Re-muting replaces the caller's previous mute for the same alert.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
