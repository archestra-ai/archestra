...
function extractContentFromDataUrl(
  ...
  try {
    ...
    if (estimatedSize > MAX_DOCUMENT_SIZE_BYTES) {
      logger.warn(
        { estimatedSize },
        "[KnowledgeBase] Skipping data URL that likely exceeds size limit",
      );
      return null;
    }
    ...
    if (containsInvalidUtf8(decoded)) {
      logger.warn(
        { filename },
        "[KnowledgeBase] Skipping file with invalid UTF-8 content",
      );
      return null;
    }
    ...
    logger.warn(
      ...
      "[KnowledgeBase] Failed to decode data URL",
    );
    return null;
  }
}
...
function extractDocumentContent(part: MessagePart): {
  ...
  if (!isSupported) {
    logger.debug(
      { mediaType, filename },
      "[KnowledgeBase] Skipping unsupported file type",
    );
    return null;
  }
    ...
    if (content) {
      ...
      if (Buffer.byteLength(content, "utf-8") > MAX_DOCUMENT_SIZE_BYTES) {
        logger.warn(
          ...
          "[KnowledgeBase] Skipping document that exceeds size limit",
        );
        return null;
      }
      return {
  ...
  logger.debug(
    ...
    "[KnowledgeBase] Could not extract content from file part",
  );
  return null;
}

/**
 * Ingest a single document into a knowledge base provider
 */
  ...
  try {
    ...
    if (result.status === "failed") {
      logger.warn(
        ...
        "[KnowledgeBase] Document ingestion failed",
      );
      return false;
    }
    ...
    logger.info(
      ...
      "[KnowledgeBase] Document ingested successfully",
    );
    return true;
  } catch (error) {
    logger.error(
      ...
      "[KnowledgeBase] Error during document ingestion",
    );
    return false;
  }
}
 ...
 * the agent's assigned knowledge base.
 *
 ...
 * @param agentId - The agent ID to look up the assigned knowledge base
 */
export async function extractAndIngestDocuments(
  ...
  logger.info(
    ...
    "[KnowledgeBase] Ingesting documents from chat",
  );

  // Create a provider instance for this agent's knowledge base
      ...
      const promise: Promise<void> = ingestDocument(provider, {
        ...
        .catch((error) => {
          logger.error(
            ...
            "[KnowledgeBase] Background document ingestion failed",
          );
        })
        .finally(() => {
  ...
  ingestWithConcurrencyLimit().catch((error) => {
    logger.error(
      { error: error instanceof Error ? error.message : String(error) },
      "[KnowledgeBase] Background document ingestion batch failed",
    );
  });
}
