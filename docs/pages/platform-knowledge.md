...
Archestra ships with these built-in connector types. Each role carries its own list of the ones it may use, under **Resource access** in Settings → Roles. A connector type a role does not have disappears from the pickers, and the API refuses to configure it. Connectors that already exist keep syncing until you delete them.
