...
-- the backend runs them once at startup after schema migration
-- (`services/resource-permissions-cutover.ts`), so sharing and role authority
-- move together in one transaction and later permission edits stay
