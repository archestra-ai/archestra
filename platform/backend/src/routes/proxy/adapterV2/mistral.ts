...
type MistralToolResultImageBlock = {
  type: "image_url";
  image_url: {
    url: string;
...
type MistralToolResultTextBlock = {
  type: "text";
  text: string;
};

type MistralToolResultContentBlock =
  | MistralToolResultImageBlock
  | MistralToolResultTextBlock;

type MistralToolResultContent = string | MistralToolResultContentBlock[];
  ...
  implements LLMRequestAdapter<MistralRequest, MistralMessages>
{
  ...
  convertToolResultContent(messages: MistralMessages): MistralMessages {
    ...
    for (const message of messages) {
      if (message.role === "tool") {
        ...
        logger.info(
          ...
          "[MistralAdapter] Analyzing tool result content",
        );
          ...
          for (const [idx, item] of message.content.entries()) {
            if (typeof item === "object" && item !== null) {
              ...
              logger.info(
                ...
                "[MistralAdapter] Tool result array item",
              );
            }
          }
        }
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to Mistral/OpenAI format
      const convertedContent = convertMcpImageBlocksToMistral(message.content);
      if (!convertedContent) {
        return message;
      }
    ...
    if (toolMessagesWithImages > 0 || strippedImageCount > 0) {
      logger.info(
        ...
        "[MistralAdapter] Processed tool messages with image content",
      );
    }
  ...
  toProviderRequest(): MistralRequest {
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[MistralAdapter] Stripped browser tool results",
        );
      }
    }
    ...
    logger.info(
      ...
      "[MistralAdapter] Building provider request",
    );
  ...
  private toCommonFormat(messages: MistralMessages): CommonMessage[] {
    logger.debug(
      { messageCount: messages.length },
      "[MistralAdapter] toCommonFormat: starting conversion",
    );
      ...
      if (message.role === "tool") {
        ...
        if (toolName) {
          logger.debug(
            ...
            "[MistralAdapter] toCommonFormat: found tool message",
          );
    ...
    logger.debug(
      ...
      "[MistralAdapter] toCommonFormat: conversion complete",
    );
  ...
  private applyUpdates(
    ...
    logger.debug(
      ...
      "[MistralAdapter] applyUpdates: starting",
    );

    if (updateCount === 0) {
      logger.debug("[MistralAdapter] applyUpdates: no updates to apply");
      return messages;
    }
    ...
    const result = messages.map((message) => {
      if (message.role === "tool" && updates[message.tool_call_id]) {
        ...
        logger.debug(
          { toolCallId: message.tool_call_id },
          "[MistralAdapter] applyUpdates: applying update to tool message",
        );
        return {
          ...message,
    ...
    logger.debug(
      ...
      "[MistralAdapter] applyUpdates: complete",
    );
    return result;
  }
}

function convertMcpImageBlocksToMistral(
  content: unknown,
): MistralToolResultContent | null {
  ...
  const mistralContent: MistralToolResultContentBlock[] = [];
    ...
    if (isMcpImageBlock(item)) {
      ...
      if (shouldStripImage) {
        logger.info(
          ...
          "[MistralAdapter] Stripping MCP image block due to size limit",
        );
        mistralContent.push({
          type: "text",
      ...
      logger.info(
        {
          ...
          // But for images, Mistral/OpenAI uses tile-based calculation
          estimatedBase64Tokens: Math.round(base64Length / 4),
        },
        "[MistralAdapter] Converting MCP image block to Mistral format",
      );
  ...
  logger.info(
    {
      totalBlocks: mistralContent.length,
      imageBlocks: mistralContent.filter((b) => b.type === "image_url").length,
      textBlocks: mistralContent.filter((b) => b.type === "text").length,
    },
    "[MistralAdapter] Converted MCP content to Mistral format",
  );

  return mistralContent.length > 0 ? mistralContent : null;
}
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[MistralAdapter] Stripped images from tool result (model does not support images)",
    );
  }
  ...
  implements LLMStreamAdapter<MistralStreamChunk, MistralResponse>
{
  ...
  processChunk(chunk: MistralStreamChunk): ChunkProcessingResult {
    ...
    // Handle usage first - OpenAI/Mistral sends usage in a final chunk with empty choices[]
