...
export default function McpCatalogLayout({
  ...
  const isListPage = pathname === "/mcp/registry";
  const isInstallationRequests = pathname.startsWith(
  ...
  // Detail/edit/new/catalog pages carry their own headers — render bare content
  // (no overflow wrapper, so in-page sticky footers pin to the viewport). The
  // list and installation-requests views still use the shared PageLayout.
  const isFullPage = !isListPage && !isInstallationRequests;
  if (isFullPage) {
    return (
  ...
  // The list opens the routed setup wizard to add a server.
  const registryActionButton = isListPage ? (
    <PermissionButton
