...
function convertMcpImageBlocksToOpenAi(
  ...
  for (const item of content) {
    ...
    if (isMcpImageBlock(item)) {
      ...
      logger.info(
        ...
        "[GroqAdapter] Converting MCP image block to OpenAI format",
      );
  ...
  logger.info(
    ...
    "[GroqAdapter] Converted MCP content to OpenAI format",
  );
...
class GroqResponseAdapter implements LLMResponseAdapter<OpenAiResponse> {
  readonly provider = "groq" as const;
...
async function convertToolResultsToToon(
  ...
  // Using OpenAI tokenizer for compatibility as Groq uses similar tokenization
