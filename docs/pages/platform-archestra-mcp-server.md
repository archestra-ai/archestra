...
| `header` | `string` | No | A very short label shown as the question's tab, e.g. 'Visibility'. |
...
| `get_plugin` | Read one plugin, including every file's exact bytes and the plugin's current contentHash. | `plugin:read` |
| `create_plugin` | Create a plugin from an explicit file set. | `plugin:create` |
| `update_plugin` | Update a plugin's metadata, visibility, or entire file set. | `plugin:update` |
| `edit_plugin` | Make a targeted edit to one file of a plugin without resending the whole file set. | `plugin:update` |
| `delete_plugin` | Delete a plugin. | `plugin:delete` |
...
Required RBAC permission: `plugin:create`
...
Required RBAC permission: `plugin:update`
...
Required RBAC permission: `plugin:delete`
