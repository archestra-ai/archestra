  ...
  implements LLMRequestAdapter<CerebrasRequest, CerebrasMessages>
{
  ...
  toProviderRequest(): CerebrasRequest {
    ...
    if (config.features.browserStreamingEnabled) {
      ...
      if (sizeBeforeStrip.length !== sizeAfterStrip.length) {
        logger.info(
          ...
          "[CerebrasAdapter] Stripped browser tool results",
        );
      }
    }
    ...
    logger.info(
      ...
      "[CerebrasAdapter] Building provider request",
    );
...
function stripImageBlocksFromContent(content: unknown): string {
  ...
  if (imageCount > 0) {
    ...
    logger.info(
      { imageCount },
      "[CerebrasAdapter] Stripped images from tool result (model does not support images)",
    );
  }
  ...
  implements LLMStreamAdapter<CerebrasStreamChunk, CerebrasResponse>
{
  ...
  processChunk(chunk: CerebrasStreamChunk): ChunkProcessingResult {
    ...
    // Handle usage first - Cerebras may send usage in a final chunk
    if (chunk.usage) {
      this.state.usage = {
...
async function convertToolResultsToToon(
  ...
  // Use OpenAI tokenizer as Cerebras is OpenAI-compatible
...
> = {
  ...
  createStreamAdapter(): LLMStreamAdapter<CerebrasStreamChunk, CerebrasResponse> {
  ...
  createClient(
    ...
    // Cerebras uses OpenAI-compatible API, so we use the OpenAI SDK
  ...
  async executeStream(
    ...
    const stream = await openaiClient.chat.completions.create(cerebrasRequest);
  ...
  extractErrorMessage(error: unknown): string {
    // OpenAI SDK error structure (used by Cerebras since it's OpenAI-compatible)
