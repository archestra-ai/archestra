...
// Mock createKnowledgeGraphProvider - this is the only module we need to mock
// since we use the real PGlite database for DB operations
vi.mock("./index", () => ({
  createKnowledgeGraphProvider: mockCreateProvider,
}));
...
describe("extractAndIngestDocuments", () => {
  ...
  test("returns early when agent has no knowledge graph assigned", async ({
    makeAgent,
  }) => {
