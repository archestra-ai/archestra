...
function loadInlineAssets() {
  ...
  const read = (filename: string) => {
    ...
    try {
      ...
      // The ext-apps bundle is generated by `pnpm build:ext-apps-inline` (run by
      // build/dev and the @backend#check:ci turbo dep), not committed — a clear
      // message beats a bare ENOENT if a build path ever skips that step.
      throw new Error(
        `MCP App inline asset "${filename}" not found at ${file}. Run "pnpm build:ext-apps-inline" (it is wired into build/dev). Cause: ${String(err)}`,
      );
    }
  };
  inlineAssetsCache = {
