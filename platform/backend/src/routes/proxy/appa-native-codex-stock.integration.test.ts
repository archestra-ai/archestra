    ...
    test("uses the authenticated gateway to approve one held local execution and resumes it once", async ({
      ...
      try {
          ...
          await expect
            .poll(async () => {
              const [pending] = await db
                ...
                .where(
                  eq(schema.appaApprovalsTable.organizationId, organization.id),
                )
                .limit(1);
              return pending;
            })
        ...
        const [approval] = await db
          .select()
          .from(schema.appaApprovalsTable)
          .where(eq(schema.appaApprovalsTable.organizationId, organization.id))
          .limit(1);
        if (!approval)
    ...
    const [approval] = await db
      ...
      .where(
        and(
          ...
          eq(schema.appaApprovalsTable.tool, params.tool),
