...
describe("IdleHibernationSection", () => {
  ...
  it("disables the control when nothing is installed, instead of pretending a default", () => {
    // The catalog update cascades over install rows; over zero rows it
    // changes nothing, so an enabled control here would toast success for a
    ...
    expect(combobox).toHaveTextContent("No installed servers");
  });

  it("shows an explicit mixed state when installs diverge", () => {
    // The reinstall route can move one install's mode independently; showing
    // whichever row lists first would render "Inherit" while a sibling pins
    // the shared pod awake.
    mockUseMcpServers.mockReturnValue({
      data: [
        makeInstalledServer({
          id: "server-1",
    ...
    expect(combobox).toHaveTextContent("Mixed across installs");
  });
