...
import * as McpServerModule from "@/models/mcp-server";
// SPDX-SnippetBegin
// SPDX-SnippetCopyrightText: 2026 Archestra Inc.
// SPDX-License-Identifier: LicenseRef-Archestra-Enterprise
...
vi.mock("@/models/mcp-server", () => ({
  ...
  MCP_DEMAND_HEARTBEAT_INTERVAL_MS: 15_000,
  // SPDX-SnippetEnd
  default: {
    findById: vi.fn().mockResolvedValue(null),
    findByCatalogId: vi.fn().mockResolvedValue([]),
    findPendingInstallationsByErrorPrefix: vi.fn().mockResolvedValue([]),
    setDeploymentName: vi.fn().mockResolvedValue(undefined),
    // SPDX-SnippetBegin
    // SPDX-SnippetCopyrightText: 2026 Archestra Inc.
    // SPDX-License-Identifier: LicenseRef-Archestra-Enterprise
...
vi.mock("@/services/mcp-active-use.ee", () => ({
  mcpActiveUseTracker: {
    ...
    stamp: vi.fn(),
    stampIfEnabled: vi.fn(async () => undefined),
    stampAwaited: vi.fn(async () => undefined),
    start: vi.fn(),
    stop: vi.fn(),
...
// This suite deliberately mocks the runtime's process boundaries and does not
// initialize PGlite. Model-level lease behavior has its own integration tests;
// here the lease grants immediately unless a test overrides it.
vi.mock("@/models/mcp-deployment-lease", () => {
  class ClusterLeaseHeldError extends Error {
    constructor(scope: string, key: string) {
      super(`lease ${scope}/${key} is held`);
    }
  }
  class ClusterLeaseLostError extends Error {}
  const held = new Set<string>();
  const waiters = new Map<string, Set<() => void>>();
  const leaseKey = (params: { scope: string; key: string }) =>
    `${params.scope}/${params.key}`;
  const waitUntilAvailable = async (params: { scope: string; key: string }) => {
    const key = leaseKey(params);
    if (!held.has(key)) return;
    await new Promise<void>((resolve) => {
      const current = waiters.get(key) ?? new Set();
      current.add(resolve);
      waiters.set(key, current);
    });
  };
  const runWithLease = async <T>(
    _params: { scope: string; key: string },
    fn: (guard: {
      signal: AbortSignal;
      assertOwned: () => Promise<void>;
      runFencedMutation: <T>(fn: (tx: never) => Promise<T>) => Promise<T>;
      throwIfLost: () => void;
    }) => Promise<T>,
  ) => {
    const key = leaseKey(_params);
    if (held.has(key)) {
      throw new ClusterLeaseHeldError(_params.scope, _params.key);
    }
    held.add(key);
    try {
      return await fn({
        ...
        runFencedMutation: (fn) => fn({} as never),
        throwIfLost: vi.fn(),
      });
    } finally {
      held.delete(key);
  ...
  return {
    ...
    default: {
      waitUntilAvailable: vi.fn(waitUntilAvailable),
      withLease: vi.fn(runWithLease),
      withLeaseWhenAvailable: vi.fn(async (params, fn) => {
        await waitUntilAvailable(params);
        return runWithLease(params, fn);
      }),
    },
  };
});
...
vi.mock("./k8s-deployment", () => {
  return {
    default: class MockK8sDeployment {
      ...
      setEffectiveNetworkPolicy(policy: unknown): void {
        this.options.effectiveNetworkPolicy = policy;
      }
...
describe("McpServerRuntimeManager", () => {
  ...
  describe("reinstallSharedDeployment", () => {
    ...
    test("tears down shared Deployment for all siblings then recreates via startServer", async () => {
      ...
      // Only the Deployment is replaced. Service, Secret, registry credentials
      // and policy are declarative and startServer reconciles them in place.
      expect(stopDeployment).toHaveBeenCalledTimes(1);
      expect(deleteK8sService).not.toHaveBeenCalled();
      expect(deleteK8sSecret).not.toHaveBeenCalled();
      expect(deleteDockerRegistrySecrets).not.toHaveBeenCalled();
      expect(deleteK8sNetworkPolicy).not.toHaveBeenCalled();
  ...
  describe("startServer - cross-server secret isolation (#3148)", () => {
    ...
    test("a successful start stamps demand so a just-(re)created deployment gets a full idle window", async () => {
      ...
      // (live-reproduced).
      expect(
        vi.mocked(mcpActiveUseTracker.stampIfEnabled),
      ).toHaveBeenCalledWith(mcpServer.id);
...
describe("McpServerRuntimeManager idle hibernation", () => {
  type MockHibernationDeployment = {
    ...
    clearForeignReplicaOwner: ReturnType<typeof vi.fn>;
    markForeignReplicaOwner: ReturnType<typeof vi.fn>;
  ...
  // Idle window + the sweeper's grace: one throttled-stamp interval plus one
  // demand-heartbeat tick.
  const IDLE_CUTOFF_MS =
    IDLE_WINDOW_SECONDS * 1000 +
    McpServerModule.MCP_SERVER_LAST_USED_REFRESH_INTERVAL_MS +
    McpServerModule.MCP_DEMAND_HEARTBEAT_INTERVAL_MS;
    ...
    return {
      ...
      get statusSummary() {
        return {
          ...
          deploymentName: this.k8sDeploymentName,
      ...
      // The real method returns the state the deployment converged to, and
      // the wake only dispatches on "running" — an undefined here reads as a
      // lost finish-wake race and fails the wake retryably.
      completeWake: vi.fn().mockResolvedValue("running"),
      clearForeignReplicaOwner: vi.fn().mockResolvedValue(undefined),
      markForeignReplicaOwner: vi.fn().mockResolvedValue(undefined),
  ...
  test("the mocked last-used grace constant matches the real one (drift guard)", async () => {
    ...
    // Both terms of the sweeper's grace are mirrored here, and a missing one
    // is not a benign gap: an undefined term makes the whole idle cutoff NaN,
    // every comparison against it false, and every candidate hibernate
    // regardless of how recently it was used.
    expect(actual.MCP_DEMAND_HEARTBEAT_INTERVAL_MS).toBe(
      McpServerModule.MCP_DEMAND_HEARTBEAT_INTERVAL_MS,
    );
  });
  ...
  describe("sweeper", () => {
    ...
    test("demand arriving during the hibernate patch waits for cleanup, then wakes", async () => {
      const { manager, internals } = await makeManager();
      const deployment = makeDeployment();
      internals.mcpServerIdToDeploymentMap.set("server-1", deployment);
      vi.mocked(McpServerModel.getLatestUsageAt).mockResolvedValue(
      ...
      let enterHibernate!: () => void;
      const hibernateEntered = new Promise<void>((resolve) => {
        enterHibernate = resolve;
      });
      let finishHibernate!: () => void;
      const hibernateCanFinish = new Promise<void>((resolve) => {
        finishHibernate = resolve;
      });
      deployment.hibernate.mockImplementation(async () => {
        enterHibernate();
        await hibernateCanFinish;
        return { hibernated: true };
      });

      const sweep = internals.sweepIdleDeployments();
      await hibernateEntered;
      const demand = manager.ensureAwake("server-1");
      await Promise.resolve();
      expect(deployment.beginWake).not.toHaveBeenCalled();

      finishHibernate();
      await Promise.all([sweep, demand]);
  ...
  describe("ensureAwake", () => {
    test("cached running state is validated live when hibernation is offered", async () => {
      const { manager, internals } = await makeManager();
    ...
    test("cached running state that another replica hibernated wakes on first call", async () => {
      const { manager, internals } = await makeManager();
      const deployment = makeDeployment({ state: "running" });
      deployment.refreshState.mockImplementation(async () => {
        deployment.state = "hibernated";
      });
      internals.mcpServerIdToDeploymentMap.set("server-1", deployment);
    ...
    test("cached running state remains zero-cost when hibernation is not offered", async () => {
      vi.mocked(
        OrganizationModel.getMcpIdleHibernationEnabledSync,
      ).mockReturnValue(false);
      const { manager, internals } = await makeManager({ betaEnabled: false });
      const deployment = makeDeployment({ state: "running" });
      internals.mcpServerIdToDeploymentMap.set("server-1", deployment);
      ...
      expect(deployment.readLiveDeployment).not.toHaveBeenCalled();
    });

    test("passive discovery skips a deployment whose awake cache is stale", async () => {
      const { manager, internals } = await makeManager();
      const deployment = makeDeployment({ state: "running" });
      deployment.refreshState.mockImplementation(async () => {
        deployment.state = "hibernated";
      ...
      internals.mcpServerIdToDeploymentMap.set("server-1", deployment);
      const operation = vi.fn(async () => "resources");

      const result = await manager.runIfDeploymentServing(
        "server-1",
        operation,
      );
    ...
    test("passive discovery holds the transition gate through its request", async () => {
      const { manager, internals } = await makeManager();
      const deployment = makeDeployment({ state: "running" });
      internals.mcpServerIdToDeploymentMap.set("server-1", deployment);
      vi.mocked(McpServerModel.getLatestUsageAt).mockResolvedValue(
      ...
      const passive = manager.runIfDeploymentServing("server-1", async () => {
        startOperation();
    ...
    test("a deployment already asleep stays dormant after hibernation is disabled", async () => {
      vi.mocked(
        OrganizationModel.getMcpIdleHibernationEnabledSync,
      ).mockReturnValue(false);
      const { manager, internals } = await makeManager({ betaEnabled: false });
    ...
    test("a later call against a deployment left failed re-reads the cluster and re-raises the failure", async () => {
      // The bug this pins: "failed" was the one cached state ensureAwake
      // treated as "already awake", so the call AFTER a terminal wake failure
      // short-circuited, skipped the wake entirely, and let the caller dispatch
      // at a pod that answers nothing. The tool call came back as a transport
      // timeout naming nothing anyone could act on, instead of the pod's own
      // reason the first caller got.
      const { manager, internals } = await makeManager();
      const { McpServerDeploymentFailedError } = await import(
        "./k8s-deployment"
      );
      const deployment = makeDeployment({ state: "failed" });
      deployment.errorMessage = "CrashLoopBackOff - server exited on boot";
      // Cluster truth agrees it is still broken.
      deployment.refreshState.mockImplementation(async () => {
      ...
      // Cluster-read, not cache-trusted.
      expect(deployment.refreshState).toHaveBeenCalled();
      expect(error).toBeInstanceOf(McpServerDeploymentFailedError);
      expect(error.message).toContain("CrashLoopBackOff");
      expect(error.message).toContain("mcp-sleepy");
      // Nothing was scaled: a broken deployment is not a sleeping one.
      expect(deployment.beginWake).not.toHaveBeenCalled();
      expect(deployment.completeWake).not.toHaveBeenCalled();
      expect(internals.wakeInFlightByPhysicalKey.size).toBe(0);
    });

    test("a deployment that recovered since we cached failed is woken, not refused", async () => {
      // The mirror of the test above: distrusting the cached "failed" has to
      // cut both ways, or an operator fixing the image would be locked out by
      // a stale cache until something else happened to refresh it.
      const { manager, internals } = await makeManager();
      const deployment = makeDeployment({ state: "failed" });
      deployment.refreshState.mockImplementation(async () => {
        deployment.state = "hibernated";
    ...
    test("a wake attempt that never settles releases its waiters and frees the slot for a fresh attempt", async () => {
      ...
      try {
        const { manager, internals, McpServerWakeError } = await makeManager();
        const { MCP_DEPLOYMENT_TRANSITION_DEADLINE_MS } = await import(
    ...
    test("a successful wake stamps the group so the next sweep sees a full idle window", async () => {
      ...
      expect(
        vi
          .mocked(mcpActiveUseTracker.stampIfEnabled)
          .mock.calls.map((c) => c[0]),
      ).toEqual(["sib-a"]);
    });
