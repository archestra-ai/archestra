...
describe("BrowserTabStateModel", () => {
  ...
  describe("deleteByIsolationKey", () => {
    test("deletes all states for an isolation key", async ({
      makeAgent,
      makeUser,
    }) => {
      const agent1 = await makeAgent();
      ...
      await BrowserTabStateModel.deleteByIsolationKey("conv-1");
    ...
    test("does not affect other isolation keys", async ({
      makeAgent,
      makeUser,
    }) => {
      const agent = await makeAgent();
      ...
      expect(result1).toBeNull();
      expect(result2?.url).toBe("https://conv2.com");
    });
  });
});
