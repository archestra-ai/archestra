...
vi.mock("../ui/card", () => ({
  ...
  CardContent: vi.fn(({ children, ...props }) => (
    <div data-testid="CardContent" {...props}>
      {children}
    </div>
  )),
  CardDescription: vi.fn(({ children, ...props }) => (
    <div data-testid="CardDescription" {...props}>
      {children}
    </div>
  )),
  ...
  CardHeader: vi.fn(({ children, ...props }) => (
    <div data-testid="CardHeader" {...props}>
      {children}
    </div>
  )),
  CardTitle: vi.fn(({ children, ...props }) => (
    <div data-testid="CardTitle" {...props}>
      {children}
    </div>
  )),
}));
...
describe("renderComponentTree", () => {
  ...
  it("should display an error for an unregistered component", () => {
    ...
    expect(
      screen.getByText(/Error: Component 'NonExistentComponent' not found/i),
    ).toBeInTheDocument();
  });

  it("should display an error for invalid JSON", () => {
    const invalidJson =
      '{ component: "Card", props: { children: "Missing quote }';

    render(renderComponentTree(invalidJson));
    // Updated assertion to match the new error message from renderComponentTree
    expect(
      screen.getByText(/Error: Failed to load component/i),
    ).toBeInTheDocument();
  });
  ...
  it("should correctly render nested objects as JSON strings when passed as children", () => {
    ...
    expect(cardContent.textContent?.replace(/\s/g, "")).toEqual(
      JSON.stringify(complexData, null, 2).replace(/\s/g, ""),
    );
  });
