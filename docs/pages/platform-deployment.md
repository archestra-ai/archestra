...
- **`ARCHESTRA_LLM_PROXY_AUTH_RATE_LIMIT_MAX_FAILURES`** - Failed virtual-key validations allowed per client IP and credential, per window, before the proxy answers `429`.
  - Default: `10`
  - Only failures count. A working credential never approaches this limit
  - The `429` carries a `Retry-After` header naming the seconds left in the window

- **`ARCHESTRA_LLM_PROXY_AUTH_RATE_LIMIT_MAX_FAILURES_PER_IP`** - Failed virtual-key validations allowed per client IP across all credentials, per window. This is the anti-enumeration backstop.
  - Default: `100`
  - Behind a reverse proxy or ingress with `ARCHESTRA_TRUST_PROXY` unset, every external request shares one client IP, so this ceiling applies to the whole deployment. Raise it if legitimate authentication churn reaches it
  - A credential that validated in the last 5 minutes is exempt, so one noisy client cannot throttle everyone else

- **`ARCHESTRA_LLM_PROXY_AUTH_RATE_LIMIT_WINDOW_MS`** - Length of the fixed window both limits count within, in milliseconds.
  - Default: `60000` (1 minute)
  - Counters reset when the window ends. Failures inside an open window do not extend it
