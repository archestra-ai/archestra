...
User OAuth tokens do not use the registered clients under **Client Credentials > OAuth Clients**. That page registers confidential clients (a `client_id` with a secret). User OAuth apps are public clients registered dynamically, and provider keys are resolved from the signed-in user's accessible Model Provider keys.
