  ...
  const renderPropValue = (
    ...
    if (Array.isArray(value)) {
      return value.map((item, index) =>
        renderPropValue(item, key !== undefined ? `${key}-${index}` : index),
      );
    }
    ...
    if (
      ...
      value !== null &&
      "component" in value &&
      typeof (value as { component: string }).component === "string"
    ) {
...
export function renderComponentTree(jsonTree: string): React.ReactNode {
  try {
    ...
    if (typeof data !== "object" || data === null) {
      return (
        <div className="text-destructive">
          Error: Invalid component tree data.
        </div>
      );
    }
    ...
    if (data.component && typeof data.component === "string") {
      if (componentRegistry[data.component]) {
        ...
        return (
          <div className="text-destructive">
            Error: Component '{data.component}' not found.
          </div>
        );
      }
      ...
      return (
        <>
          {data.map((item) => {
            // Generate a stable key based on item content
            const generateKey = (): string => {
              if (typeof item === "object" && item !== null) {
                if ("component" in item && typeof item.component === "string") {
                  return `component-${item.component}-${JSON.stringify(item.props || {})}`;
                }
            ...
            if (
              ...
              item !== null &&
              "component" in item &&
              typeof (item as { component: string }).component === "string"
            ) {
              if (
                componentRegistry[(item as { component: string }).component]
              ) {
                return React.createElement(DynamicReactRenderer, {
                  componentName: (item as { component: string }).component,
                  props:
                ...
                return (
                  <div key={itemKey} className="text-destructive">
                    Error: Component '
                    {(item as { component: string }).component}' not found in
                    array.
                  </div>
                );
              }
            ...
            return (
              <div key={itemKey} className="text-destructive">
                Error: Invalid item in component tree array.
              </div>
            );
          })}
        </>
    ...
    return (
      <div className="text-destructive">Error: Invalid component format.</div>
    );
  } catch (error) {
    console.error("Failed to parse component JSON:", error);
    return (
      <div className="text-destructive">Error: Failed to load component.</div>
    );
  }
}
