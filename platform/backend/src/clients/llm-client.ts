...
export function detectProviderFromModel(model: string): SupportedChatProvider {
  ...
  if (lowerModel.includes("deepseek") || lowerModel.includes("llama") || lowerModel.includes("mixtral") || lowerModel.includes("gemma")) {
    return "deepseek";
  }
...
export const FAST_MODELS: Record<SupportedChatProvider, string> = {
  ...
  deepseek: "deepseek-chat", // DeepSeek's fast versatile model
