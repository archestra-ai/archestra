...
export class AppaProxyHookSession {
  ...
  private async prepareInboundResults(
    ...
    for (let result of toolResults) {
      ...
      if (this.nativeCodexExecution && call.emittedName.startsWith("mcp__")) {
        const receipt =
        ...
        // The client only correlates this result. Its status is derived from
        // provider-specific result framing and is not an authority over the
        // authenticated gateway receipt. The outer wrapper and arguments were
        // already bound above; only the gateway's persisted outcome is admitted.
        result =
          receipt.status === "indeterminate"
            ? {
                ...
                // A local client can report an approval refusal without a
                // gateway invocation. It is not authenticated tool output and
                // must never become provider-visible business data.
                content: nativeMcpUnattestedResult(),
              }
            : {
                ...result,
                status: receipt.status,
                ...
                content: JSON.stringify({
                  ...
                  structuredContent: receipt.result.structuredContent,
  ...
  private async ensureV1Capabilities(): Promise<void> {
    ...
    if (
      ...
      capabilities.parallel_calls !== true ||
  ...
  > {
    if (!this.capabilities.parallelCalls) {
      throw new AppaProxyHookError("unavailable", "outbound");
    }
...
function nativeMcpUnattestedResult(): string {
  return JSON.stringify({
    content: [
      {
        type: "text",
        text: "Native MCP execution was not authenticated by the gateway.",
      },
    ],
    isError: true,
  });
