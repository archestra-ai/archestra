 ...
 * x.AI Adapter
 *
 * x.AI exposes an OpenAI-compatible API, so this adapter is largely based on the OpenAI adapter.
  ...
  implements LLMRequestAdapter<XaiRequest, XaiMessages>
{
  ...
  convertToolResultContent(messages: XaiMessages): XaiMessages {
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to x.AI/OpenAI format
  ...
  implements LLMStreamAdapter<XaiStreamChunk, XaiResponse>
{
  ...
  processChunk(chunk: XaiStreamChunk): ChunkProcessingResult {
    ...
    // Handle usage first - x.AI (like OpenAI) sends usage in a final chunk with empty choices[]
...
async function convertToolResultsToToon(
  ...
  // Use OpenAI tokenizer since x.AI uses similar tokenization for most models
...
> = {
  ...
  extractApiKey(headers: XaiHeaders): string | undefined {
    // x.AI typically doesn't require API keys, but support them if provided
  ...
  createClient(
    ...
    // x.AI uses OpenAI SDK since it's OpenAI-compatible
    // Use dummy API key if none provided (x.AI typically doesn't require auth)
  ...
  extractErrorMessage(error: unknown): string {
    // x.AI uses OpenAI SDK error structure
