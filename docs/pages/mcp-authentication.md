...
Set **Protected Resource** when the OAuth resource identifier differs from the MCP endpoint URL, such as an Entra or OAuth API identifier like `api://<client-id>` or `https://api.example.com`. Archestra sends this as the OAuth `resource` parameter. Token exchange and refresh requests only send `resource` when **Protected Resource** or **Default Audience** is configured; otherwise Archestra omits it for providers that do not accept resource indicators.
