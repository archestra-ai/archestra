...
/**
 * `status` and `summary` arrived long after the 40-odd call sites that pass
 * neither, so what they must not do is as much of the contract as what they do.
 */
describe("PageLayout header", () => {
  ...
  it("renders a legacy caller's header with nothing extra in it", () => {
    ...
    // No empty status or summary shells left behind for a caller that passes
    // neither — an "Active" pill on every record is the row this prop exists
    // to keep optional.
  ...
  it("renders the summary under the description, outside the heading", () => {
    render(
      <PageLayout
        title="Support Bot"
        description="Answers billing questions."
        summary="Aug 20, 2026 by Admin"
      >
        <div />
      </PageLayout>,
    );

    const summary = screen.getByText("Aug 20, 2026 by Admin");
    ...
    expect(
      screen
        .getByText("Answers billing questions.")
