...
/**
 * Only the types that actually pull from a source system. `file_upload` is
 * absent by design — its documents are pushed in by the knowledge-files page,
 * so there is nothing to sync FROM and no implementation to register.
 * `getConnector` throws for it, which is the correct answer to "sync this".
 */
const connectorRegistry: Record<UserSelectableConnectorType, () => Connector> =
  {
...
export function getConnector(type: string): Connector {
  const factory = connectorRegistry[type as UserSelectableConnectorType];
  if (!factory) {
...
export function getPermissionSyncConnectorTypes(): ConnectorType[] {
  if (permissionSyncConnectorTypes === null) {
    permissionSyncConnectorTypes = (
      Object.keys(connectorRegistry) as UserSelectableConnectorType[]
