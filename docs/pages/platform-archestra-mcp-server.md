...
| Tool | Description | Required RBAC Permission |
|------|-------------|--------------------------|
| `start_runner` | Start a long-running agent session in its own container for an agent that has runner configuration. | `runner:create` |
| `list_runners` | List agent sessions in the organization, optionally filtered by agent or state. | `runner:read` |
| `get_runner` | Get one agent session with its recent timeline: state changes, steer messages and lifecycle notices. | `runner:read` |
| `send_to_runner` | Send a message into a live agent session. | `runner:update` |
| `stop_runner` | Stop a live agent session and remove its container. | `runner:update` |

#### start_runner

Required RBAC permission: `runner:create`

##### Input

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `agentId` | `string` | Yes | ID of the agent to run. |
| `name` | `string` | Yes | Short human-readable name for this session. |
| `task` | `string` | No | Initial instruction handed to the agent on start. |
...
|-------|------|----------|-------------|
| `runner` | `object` | Yes |  |
| `runner.id` | `string` | Yes | The runner ID. |
| `runner.name` | `string` | Yes | The runner's display name. |
| `runner.agentId` | `string` | Yes | The agent this runner is running. |
| `runner.state` | `"pending" \| "provisioning" \| "running" \| "stopping" \| "stopped" \| "failed"` | Yes | Current lifecycle state. |
| `runner.statusReason` | `string \| null` | Yes | Why the runner is in its current state, when known. |
| `runner.image` | `string` | Yes | Container image the session is running. |
| `runner.createdByUserId` | `string` | Yes | The user this runner acts on behalf of. |
| `runner.createdAt` | `string` | Yes | When the runner was created (ISO 8601). |

#### list_runners

Required RBAC permission: `runner:read`

##### Input

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `agentId` | `string` | No | Only list runners for this agent. |
| `state` | `"pending" \| "provisioning" \| "running" \| "stopping" \| "stopped" \| "failed"` | No | Only list runners in this state. |
| `mine` | `boolean` | No | Only list runners you started. |
...
|-------|------|----------|-------------|
| `runners` | `object[]` | Yes |  |
| `runners[].id` | `string` | Yes | The runner ID. |
| `runners[].name` | `string` | Yes | The runner's display name. |
| `runners[].agentId` | `string` | Yes | The agent this runner is running. |
| `runners[].state` | `"pending" \| "provisioning" \| "running" \| "stopping" \| "stopped" \| "failed"` | Yes | Current lifecycle state. |
| `runners[].statusReason` | `string \| null` | Yes | Why the runner is in its current state, when known. |
| `runners[].image` | `string` | Yes | Container image the session is running. |
| `runners[].createdByUserId` | `string` | Yes | The user this runner acts on behalf of. |
| `runners[].createdAt` | `string` | Yes | When the runner was created (ISO 8601). |
...
| `runner.createdAt` | `string` | Yes | When the runner was created (ISO 8601). |
| `events` | `object[]` | Yes |  |
| `events[].sequence` | `number` | Yes |  |
| `events[].kind` | `string` | Yes |  |
| `events[].message` | `string \| null` | Yes |  |
| `events[].createdAt` | `string` | Yes |  |

#### send_to_runner

Required RBAC permission: `runner:update`
