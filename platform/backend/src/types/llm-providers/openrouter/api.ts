 ...
 *
 * OpenRouter uses an OpenAI-compatible API with some differences:
...
const OpenRouterChoiceSchema = z
  .object({
    ...
    message: z
      .object({
        // OpenRouter: content is optional when tool_calls are present
      ...
      .describe(`https://openrouter.ai/docs/api-reference`),
  })
  .describe(`https://openrouter.ai/docs/api-reference`);
