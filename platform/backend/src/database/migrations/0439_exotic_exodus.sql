...
-- drizzle-migration-linter: reason=All three tables (runners, runner_events, user_credentials) are created empty in this same file, so every foreign key, unique index and non-concurrent index validates against zero rows and takes no meaningful lock. No older writer can produce a row in them. ON DELETE cascade is intentional in each case: a runner must not outlive the agent it runs or the user whose personal credentials it holds, a runner's event timeline must not outlive the runner, and a personal credential must not outlive its user or its secret. The two set-null foreign keys are deliberate instead: a runner keeps its history when its environment or its minted virtual key is removed.
...
CREATE UNIQUE INDEX "user_credentials_org_user_key_uidx" ON "user_credentials" USING btree ("organization_id","user_id","key");