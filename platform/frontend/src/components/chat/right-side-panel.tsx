  ...
  return (
    <ResizableRightPanel>
      <div className="flex-1 min-h-0 overflow-hidden relative">
        ...
        {resolvedTab === "apps" && (
          <AppsPanelContent agentId={agentId} conversationId={conversationId} />
        )}
      </div>
    </ResizableRightPanel>
  );
 ...
 */
export function AppsPanelContent({
  agentId,
  conversationId,
}: {
  agentId?: string;
    ...
    <div className="flex flex-col h-full">
      <div ref={portalDivRef} className="flex-1 min-h-0 relative">
        {apps.length === 0 ? (
          ...
          <PanelAppHost agentId={agentId} conversationId={conversationId} />
        )}
      </div>
    </div>
  );
