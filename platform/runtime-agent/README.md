...
The agent loop that runs inside an Archestra Agent's Agent Runtime
...
the isolated run workspace, prints activity legibly for anyone attached
...
An Agent Runtime image must provide `tmux` and a POSIX shell — the runtime makes tmux
PID 1, and that is what makes a session attachable. An image that also puts
`archestra-runtime-agent` on `PATH` can be started with no command at all;
anything else supplies its own command in the Agent's Agent Runtime
configuration.
...
| Variable | Meaning |
| --- | --- |
| `ARCHESTRA_AGENT_RUNTIME_AGENT_ID`, `ARCHESTRA_AGENT_RUNTIME_AGENT_NAME` | Agent identity for this run |
| `ARCHESTRA_AGENT_RUNTIME_TASK_ID` | Durable run id, attached to LLM and MCP requests for log and trace correlation |
| `ARCHESTRA_LLM_PROXY_URL`, `ARCHESTRA_LLM_PROXY_PROTOCOL`, `ARCHESTRA_VIRTUAL_KEY` | The Agent-scoped proxy, its Responses, Chat Completions, or Anthropic Messages wire protocol, and the run's short-lived personal virtual key |
| `ARCHESTRA_MCP_GATEWAY_URL`, `ARCHESTRA_MCP_GATEWAY_TOKEN` | Tool access, as the invoking user |
| `ARCHESTRA_AGENT_RUNTIME_TASK` | Initial instruction, when started with one |
| `ARCHESTRA_AGENT_RUNTIME_ATTACHMENTS_DIR`, `ARCHESTRA_AGENT_RUNTIME_ATTACHMENTS_MANIFEST` | Files attached to the initial instruction and their JSON metadata |
| `ARCHESTRA_AGENT_RUNTIME_STEER_FIFO` | Where steer messages arrive |
| `ARCHESTRA_AGENT_RUNTIME_MODEL`, `ARCHESTRA_AGENT_RUNTIME_NATIVE_MODEL`, `ARCHESTRA_AGENT_RUNTIME_MAX_STEPS` | Provider-qualified model for the built-in runtime, native model slug for catalog CLIs, and step cap |
