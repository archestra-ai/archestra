...
| Page IDs     | Comma-separated specific page IDs to sync (optional)                        |
| Batch Size   | Items per batch (default: 100). Controls how many items are fetched per sync request. |

The Notion connector processes content in batches. You can configure the batch size in the connector settings; the default is 100 items per sync request.
