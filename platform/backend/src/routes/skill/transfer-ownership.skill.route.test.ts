...
describe("POST /api/skills/:id/transfer-ownership", () => {
  ...
  beforeEach(async ({ makeOrganization, makeUser, makeMember }) => {
    ...
    create = async (ownerId = user.id, name = "handoff-resource") => {
      const row = await SkillModel.createWithFiles({
        skill: {
          organizationId,
          authorId: ownerId,
          name,
      ...
      if (!row) throw new Error("Missing skill");
      return row;
    };
  ...
  const snapshot = (id: string) =>
    SkillModel.findByIdForAudit(id, organizationId);
  ...
  test("rejects a stale ownership snapshot", async () => {
    ...
    expect((await transfer(resource.id)).statusCode).toBe(200);
    expect(
      await SkillModel.transferOwnership({
        id: resource.id,
        organizationId,
  ...
  test("refuses platform-managed skills", async () => {
    const row = await SkillModel.createWithFiles({
      skill: {
        organizationId,
