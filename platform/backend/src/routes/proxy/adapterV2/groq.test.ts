...
describe("GroqResponseAdapter", () => {
  describe("getToolCalls", () => {
    test("converts function tool calls to common format", () => {
      ...
      const adapter = groqAdapterFactory.createResponseAdapter(response);
...
describe("GroqRequestAdapter", () => {
  describe("getModel", () => {
    test("returns original model by default", () => {
      ...
      const adapter = groqAdapterFactory.createRequestAdapter(request);
      expect(adapter.getModel()).toBe("llama3-8b-8192");
    });
    ...
    test("returns modified model after setModel", () => {
      ...
      const adapter = groqAdapterFactory.createRequestAdapter(request);
      adapter.setModel("llama3-8b-8192");
      expect(adapter.getModel()).toBe("llama3-8b-8192");
    });
  });
...
describe("groqAdapterFactory", () => {
  describe("extractApiKey", () => {
    test("returns authorization header as-is (Bearer token)", () => {
      const headers = { authorization: "Bearer gsk-test-key-123" };
      const apiKey = groqAdapterFactory.extractApiKey(headers);
      expect(apiKey).toBe("Bearer gsk-test-key-123");
    });

    test("returns authorization header as-is (non-Bearer)", () => {
      const headers = { authorization: "gsk-test-key-123" };
      const apiKey = groqAdapterFactory.extractApiKey(headers);
      expect(apiKey).toBe("gsk-test-key-123");
    });
  ...
  describe("provider info", () => {
    test("has correct provider name", () => {
      expect(groqAdapterFactory.provider).toBe("groq");
    });

    test("has correct interaction type", () => {
      expect(groqAdapterFactory.interactionType).toBe(
