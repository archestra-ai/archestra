...
Read get_run with the saved session ID. Read requests for the original goal and current turn before interpreting a short follow-up; terminal output alone may show only setup commands. Report what completed, what remains, and any failed checks.

Copy deliverables with transfer_workspace_file using workspace-relative paths (for example, reports/summary.md). It authorizes the transfer and returns a command to run in a shell; the bytes move directly and never enter this conversation. Use it for every file you are copying rather than reading, including small ones and binaries, and re-run the same command to resume an interrupted download. Read a file with read_workspace_file only when you need its contents in the conversation. That path is capped at 4 MiB and its output can be truncated, so never treat a partial response as a retrieved deliverable. In a client without a shell, read_workspace_file and write_workspace_file remain the only option, and a file above their limit cannot be moved that way.

If work will continue locally, coordinate a stopping point with the runtime and verify it stopped writing before applying its files. cancel_run stops active work while preserving the workspace, but a returned cancellation does not by itself prove the process stopped writing. transfer_workspace_file pins the file when it authorizes the transfer, so a download started at that moment stays consistent even if the runtime writes again; a mismatched checksum fails the transfer rather than delivering a damaged file. Download results before retention expires. Never delete a workspace as part of handoff.
