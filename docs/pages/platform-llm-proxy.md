...
Every configuration change to a proxy is kept as a version. Open **Version history** from the proxy's row to browse versions, read what changed, and restore an earlier one. Restoring creates a new version — the history is never rewritten. See [Version History](/docs/platform-agents#version-history).
