...
-- drizzle-migration-linter: reason=every flagged statement (FK, unique index, plain index, cascade) targets the brand-new empty mcp_catalog_versions table created in this migration; the only existing-table change is internal_mcp_catalog gaining a defaulted latest_version column, which is metadata-only. ON DELETE CASCADE is intentional: version snapshots have no consumer once the catalog row is hard-deleted (nothing pins a catalog version, same rationale as agent_versions).
