...
const badgeRoutes: FastifyPluginAsyncZod = async (fastify) => {
  fastify.get(
    '/badge/*',
    {
      schema: {
        operationId: 'getBadge',
        description: 'Generate a quality score badge for an MCP server',
        tags: ['Badge'],
        params: z.object({
          '*': z.string(),
        }),
        response: {
          200: z.string().describe('SVG badge image'),
          400: z.object({
            error: z.string(),
          }),
        },
    ...
    async (request, reply) => {
      ...
      // Expected formats:
      // /api/v1/badge/quality/github-org/repo-name
      // /api/v1/badge/quality/github-org/repo-name/path--to--server
      if (pathParts.length < 3 || pathParts[0] !== 'quality') {
        return reply.status(400).send({
          error: 'Invalid format. Use: /api/v1/badge/quality/github-org/repo-name[/path--to--server]',
        });
      }

      const [, githubOrg, repoName, ...pathSegments] = pathParts;

      // Convert path back from URL format (-- becomes /)
      const repositoryPath = pathSegments.length > 0 ? pathSegments.join('/').replace(/--/g, '/') : null;
