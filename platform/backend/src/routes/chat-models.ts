 ...
 * Groq is OpenAI-compatible, so we use the same /models endpoint format
 */
async function fetchGroqModels(apiKey: string): Promise<ModelInfo[]> {
  ...
  // Filter to only chat-compatible models that support tool calling
  // Follow the same pattern as OpenAI filtering
  ...
  return data.data
    .filter((model) => {
      ...
      // Must not contain excluded patterns (non-chat or non-tool-calling models)
      const hasExcludedPattern = excludePatterns.some((pattern) =>
        id.includes(pattern),
      );
      return !hasExcludedPattern;
