...
test.describe("MCP UI Extended Tests", () => {
  test.beforeAll(async ({ request, makeApiRequest }) => {
    ...
    if (!catalogItem)
      throw new Error(`Catalog item '${MCP_UI_SERVER_ACTIONS}' not found.`);
    const server = await findInstalledServer(request, catalogItem.id);
    if (!server) throw new Error("Failed to install or find MCP server.");
    await waitForServerInstallation(request, server.id);

    const defaultProfileRes = await makeApiRequest({
      request,
      method: "get",
      urlSuffix: "/api/agents/default",
    });
    const defaultProfile = await defaultProfileRes.json();
    const toolsRes = await makeApiRequest({
      request,
      method: "get",
      urlSuffix: "/api/tools",
    });
    ...
    const tool = tools.find((t: { name: string }) =>
      t.name.includes(MCP_UI_SERVER_ACTIONS),
    );
  ...
  test("should handle 'tool' action", async ({ page, goToPage }) => {
    ...
    await expect(
      page.getByText('Tool executed with params: {"foo":"bar"}'),
    ).toBeVisible();
  });
  ...
  test("should handle 'intent' action", async ({ page, goToPage }) => {
    ...
    await expect(
      page.getByText('Intent executed with params: {"foo":"bar"}'),
    ).toBeVisible();
  });

  test("should render fallback table for simple object", async ({
    page,
    goToPage,
  }) => {
    await goToPage(page, "/chat");
  ...
  test("should display error and retry on failed import", async ({
    page,
    goToPage,
  }) => {
