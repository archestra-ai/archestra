...
class DesktopInstaller {
  ...
  request(req,res) {
    ...
    if (req.method === 'GET' && url.pathname === base + '/status') {res.setHeader('Content-Type','application/json');res.end(JSON.stringify({...this.state,thirdPartyInference:!!this.setup?.proxy}));return;}
...
function completionPage(setup,success) {
  ...
  const instructions = success ? 'Your connection settings are saved.' + (setup.proxy ? ' Send a message in Desktop and check LLM Proxy Logs to verify inference.' : '') : 'Your connection settings are saved, but Desktop could not restart automatically. Finish active tasks, quit Desktop from its app menu, and open it again.';
  const steps = [setup.mcp && 'In Desktop Settings → Connectors, connect your gateway if needed. Complete sign-in and approve access in your browser.',setup.mcp && 'In your conversation, open + → Connectors and enable your gateway if it is off. A connected checkmark in Settings does not confirm it is enabled for that conversation.',setup.mcp && 'Ask Claude to list the tools available from your gateway.',setup.proxy && 'To return to standard Claude Desktop, choose Anthropic sign-in on Desktop’s sign-in screen. Keep the third-party app data to preserve conversations created in that mode.',setup.skills && 'Your selected shared skills install automatically after Desktop restarts.','You can remove the setup helper from Desktop Extensions.'].filter(Boolean);
