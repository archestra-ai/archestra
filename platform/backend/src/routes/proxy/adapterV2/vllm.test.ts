...
function createMockResponse(
    ...
    return {
        ...
        model: "vllm-model", // Changed model name
        choices: [
            {
                index: 0,
                message: {
...
describe("VLLMResponseAdapter", () => {
    describe("getToolCalls", () => {
        test("converts function tool calls to common format", () => {
            ...
            const adapter = vllmAdapterFactory.createResponseAdapter(response);
...
describe("VLLMRequestAdapter", () => {
    describe("getModel", () => {
        test("returns original model by default", () => {
            const request = createMockRequest([{ role: "user", content: "Hello" }], {
                model: "vllm-model-mini",
            });

            const adapter = vllmAdapterFactory.createRequestAdapter(request);
            expect(adapter.getModel()).toBe("vllm-model-mini");
        });
        ...
        test("returns modified model after setModel", () => {
            ...
            const adapter = vllmAdapterFactory.createRequestAdapter(request);
            adapter.setModel("vllm-model-mini");
            expect(adapter.getModel()).toBe("vllm-model-mini");
        });
    });
    ...
    describe("toProviderRequest", () => {
        test("applies model change to request", () => {
            ...
            expect(result.model).toBe("vllm-model-mini");
        });
...
describe("vllmAdapterFactory", () => {
    ...
    describe("provider info", () => {
        test("has correct provider name", () => {
            expect(vllmAdapterFactory.provider).toBe("vllm");
        });

        test("has correct interaction type", () => {
            expect(vllmAdapterFactory.interactionType).toBe(
