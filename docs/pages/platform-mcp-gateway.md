...
When the request is authenticated as a specific user (not an org or team token), `search_tools` also returns third-party tools from MCP catalogs that user can access, and `run_tool` assigns such a tool to the gateway automatically on first use when the user has permission to modify the gateway. Admins can turn this off with the **Tool Auto-Assignment** security setting on the agent settings page.
