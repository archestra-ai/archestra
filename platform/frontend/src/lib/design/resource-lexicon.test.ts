...
describe("resource lexicon", () => {
  ...
  it("phrases Created as the date alone, leaving the owner to its own field", () => {
    // The author used to be appended here ("Aug 20, 2026 by Admin"), which
    // named the owner twice on a card that already carries an Owner field.
    expect(formatCreated({ createdAt: "2026-08-20T10:30:00.000Z" })).toBe(
      "Aug 20, 2026",
    );
  });
