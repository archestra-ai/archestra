...
import { A2aRemoteAgentModel, AuditLogModel } from "@/models";
import { createFastifyInstance, type FastifyInstanceWithZod } from "@/server";
import { createA2aRemoteAgent } from "@/services/a2a-outbound-registry";
import { afterEach, beforeEach, describe, expect, test } from "@/test";
import type { User } from "@/types";
import remoteRoutes from "./a2a-remote-agent.routes";
import { makeAgentCard } from "./a2a-remote-agent.test-helpers";

describe("POST /api/a2a/remote-agents/:id/transfer-ownership", () => {
  ...
  const snapshot = (id: string) =>
    A2aRemoteAgentModel.findByIdForAudit(id, organizationId);
  ...
  test("rejects a stale ownership snapshot", async () => {
    ...
    expect((await transfer(resource.id)).statusCode).toBe(200);
    expect(
      await A2aRemoteAgentModel.transferOwnership({
        id: resource.id,
        organizationId,
