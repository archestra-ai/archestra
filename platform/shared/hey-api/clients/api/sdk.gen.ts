...
/**
 * Atomically assign, unassign, and re-credential tools across agents. All-or-nothing: any validation failure applies no changes. One config version is forked per affected agent for the whole batch.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
...
/**
 * Create multiple hook files for one agent atomically. All-or-nothing: any conflict rejects the whole batch with every conflicting (event, fileName) pair listed. Forks one agent config version for the batch.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `agent:update`: Modify agent configuration and settings
 */
export const bulkCreateHooks = <ThrowOnError extends boolean = false>(options: Options<BulkCreateHooksData, ThrowOnError>) => (options.client ?? client).post<BulkCreateHooksResponses, BulkCreateHooksErrors, ThrowOnError>({
    url: '/api/hooks/bulk',
    ...options,
    headers: {
        'Content-Type': 'application/json',
