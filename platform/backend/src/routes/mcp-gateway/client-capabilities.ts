 ...
 * This store remembers the initialize declaration per profile+token so a
 * later `tools/call` can still tell whether the client answers a
...
export function clientCapabilityKey(params: {
  ...
  const { profileId, tokenId, userId } = params;
  return `${profileId}:${tokenId ?? userId ?? "anonymous"}`;
}
