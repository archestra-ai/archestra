...
export function detectProviderFromModel(model: string): SupportedChatProvider {
  ...
  if (lowerModel.includes("llama-3") || lowerModel.includes("mixtral") || lowerModel.includes("gemma") || lowerModel.includes("groq")) {
    // Note: this is a bit ambiguous but good enough for detection
    return "groq";
  }

  if (lowerModel.includes("abab") || lowerModel.includes("minimax")) {
    return "minimax";
  }
...
export function createLLMModel(params: {
  ...
  if (provider === "minimax") {
    // URL format: /v1/minimax/:agentId/v1 (SDK appends /chat/completions)
    const client = createOpenAI({
      apiKey,
      baseURL: `http://localhost:${config.api.port}/v1/minimax/${agentId}/v1`,
      headers,
    });
    return client.chat(modelName);
  }
