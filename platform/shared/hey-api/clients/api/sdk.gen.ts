...
/**
 * Create a folder in the calling user's persistent file storage (X-Files). Flat — folders cannot nest.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `sandbox:execute`: Run commands and upload/download files in code execution sandboxes
 */
export const createSkillSandboxFolder = <ThrowOnError extends boolean = false>(options: Options<CreateSkillSandboxFolderData, ThrowOnError>) => (options.client ?? client).post<CreateSkillSandboxFolderResponses, CreateSkillSandboxFolderErrors, ThrowOnError>({
...
/**
 * Stream the raw bytes of a sandbox upload (e.g. a persistent file the agent pulled into the sandbox). Inline for known-safe raster images; download for everything else.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `sandbox:execute`: Run commands and upload/download files in code execution sandboxes
 */
export const getSkillSandboxUpload = <ThrowOnError extends boolean = false>(options: Options<GetSkillSandboxUploadData, ThrowOnError>) => (options.client ?? client).get<GetSkillSandboxUploadResponses, unknown, ThrowOnError>({ url: '/api/skill-sandbox/uploads/{uploadId}', ...options });
