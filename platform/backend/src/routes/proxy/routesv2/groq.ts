...
import { constructResponseSchema, UuidIdSchema, Groq } from "@/types";
          ...
          logger.info(
            {
              ...
              finalProxyUrl: `${config.llm.groq.baseUrl}/v1${remainingPath}`,
            },
          ...
          logger.info(
            {
              ...
              url: request.url,
              upstream: config.llm.groq.baseUrl,
              finalProxyUrl: `${config.llm.groq.baseUrl}/v1${pathAfterPrefix}`,
            },
    ...
    async (request, reply) => {
      if (!config.llm.groq.enabled) {
        return reply.status(500).send({
          error: {
            message:
              "Groq provider is not configured. Set ARCHESTRA_GROQ_API_KEY to enable.",
