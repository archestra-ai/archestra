...
const registry = defineArchestraTools([
  defineArchestraTool({
    ...
    title: "Run Tool",
    description:
      "Dispatch to any tool available to this agent — built-in Archestra tools (e.g. 'archestra__whoami', 'archestra__get_agent' or short names like 'whoami', 'get_agent'), agent delegation tools ('agent-<id>'), or third-party MCP tools exposed through the MCP Gateway (e.g. 'context7__resolve-library-id'). Pass the tool name exactly as it appears in the tools list, and prefer using search_tools first when you need to discover the right exact name. Target-tool RBAC, argument validation, and output validation all still apply.",
