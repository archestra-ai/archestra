...
- **Never throw on HTTP errors**: In query/mutation functions, never throw errors on HTTP failures. Use `showErrorToastFromApiError(error)` for user notification and return appropriate default values (`null`, `[]`, `{}`). Components should not have try/catch for API calls - all error handling belongs in `.query.ts` files.
