//! `baton-agent`: a rig-core agent that plays the external harness. It talks
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let gateway = ElicitingClient.serve(transport).await.map_err(|e| {
        format!(
...
impl ClientHandler for ElicitingClient {
    fn get_info(&self) -> ClientInfo {
        ...
        info.client_info = Implementation::new("baton-agent", env!("CARGO_PKG_VERSION"));
        info
    }
