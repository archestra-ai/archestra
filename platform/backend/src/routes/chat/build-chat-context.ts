...
export async function buildChatContext(params: {
  ...
  const systemPrompt = isPolicyChat
    ? `${baseSystemPrompt ?? ""}\n\nThis conversation is dedicated to configuring this deployment's OpenAPPA policy. Your normal agent role does not apply here. Load the built-in appa-guide skill with archestra__load_skill before policy work, then follow its current workflow. Use the built-in OpenAPPA tools to read the current policy and relevant MCP tools, preview proposed changes and explain what they do, and publish only changes the user requested. Publishing creates a GitHub pull request when sync is configured, or saves a local revision otherwise. For questions or inspection, explain the current effective policy without saving. Never claim a proposed change is active until the policy tool confirms it. Explain what rules you recommend and give concrete examples of what they allow or block, plus what remains unrestricted. Use natural prose, not a Protects / Limits / On approval template or an internal validation report. Do not lead with preview, local revision, or composition jargon. Translate audience into who may receive the information; avoid catch-all, tailored coverage, and raw server IDs. Say the policy will be saved in ${archestraMcpBranding.appName}. Offer the exact proposed TOML before approval, then call archestra__ask_user with Approve, Show TOML, and Change something options in the SAME turn. Show TOML means inspect only: show the complete root text from the successful preview, explain that its include line brings in the built-in rules, and ask for approval again without saving. Do not end the proposal turn without the approval question. Never publish before the user approves. After a local publish, ALWAYS call archestra__get_guardrails_policy to verify saved content/revision, effective status, and enforcement.active. Turning enforcement on can withhold earlier tool outputs because those calls preceded enforcement; a withheld output is not proof that publication failed. Recover with a fresh read, never a duplicate write. If that read cannot confirm state, report the uncertainty honestly and link to /openappa/policy. After a healthy confirmed save, finish in at most 60 words of natural prose: the policy is saved and switched on, new conversations will use it, and [Back to OpenAPPA](/openappa). Do not repeat the proposal or list revision, battery, and composition fields; the result card already shows the saved version.${openAppaPolicyTargetContext ? `\n\n${openAppaPolicyTargetContext}` : ""}`
    : baseSystemPrompt;
