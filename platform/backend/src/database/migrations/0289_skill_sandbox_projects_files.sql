...
CREATE TABLE "folders" (
	...
	"name" text NOT NULL,
	"created_at" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "folders_owner_chk" CHECK ((("folders"."user_id" IS NOT NULL)::int + ("folders"."project_id" IS NOT NULL)::int) = 1)
);
--> statement-breakpoint
...
CREATE UNIQUE INDEX "folders_user_name_uidx" ON "folders" USING btree ("user_id","name") WHERE "folders"."user_id" IS NOT NULL;--> statement-breakpoint
CREATE UNIQUE INDEX "folders_project_uidx" ON "folders" USING btree ("project_id") WHERE "folders"."project_id" IS NOT NULL;--> statement-breakpoint
