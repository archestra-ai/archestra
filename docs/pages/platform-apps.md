...
An app that needs a browser device API declares it in its UI permissions — `camera`, `microphone`, `geolocation`, or `clipboardWrite`. The platform delegates each declared feature through both sandbox iframes via Permissions-Policy `allow` attributes; the browser still asks the viewer for consent on first use. Undeclared features stay blocked. Delegation requires the sandbox to run on a dedicated origin (a configured sandbox domain, or the localhost origin swap in local dev); on the same-origin fallback the sandbox is an opaque origin, which browsers refuse to grant powerful features to, so these APIs stay unavailable there regardless of the declaration.
