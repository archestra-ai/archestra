...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `providerApiKeyId` | `string \| string` | No | Filter by a configured provider key, or organization-default for agents with no pinned key or model. |
...
| `runtime` | `"dedicated" \| "foreground"` | Yes | Where the delegated task executes. |
...
| `session` | `object \| null` | Yes | The Agent run, when the task uses Agent Runtime. |
