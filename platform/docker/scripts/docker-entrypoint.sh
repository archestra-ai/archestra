...
if [ "$ARCHESTRA_QUICKSTART" = "true" ]; then
    ...
    # Pin `localhost` to IPv4.
    #
    # Docker puts both `127.0.0.1 localhost` and `::1 localhost` in /etc/hosts,
    # getaddrinfo ranks ::1 first per RFC 6724 regardless of file order, and
    # Node >= 17 passes that order through verbatim. Every service here binds
    # 0.0.0.0, so `localhost` always tries a guaranteed-refused ::1 first --
    # `wget http://localhost:9000/health` fails against a healthy backend, and
    # the frontend's API proxy logs `ECONNREFUSED ::1:9000`. Issue #4917 fixed
    # only the healthcheck's use of `localhost`; this covers the rest.
    #
    # Quickstart-only on purpose: this file is also the ENTRYPOINT of Helm's
    # platform Deployment, where /etc/hosts is shared with sidecars and
    # regenerated by kubelet on every container start. Quickstart is one
    # container on a laptop, where IPv4-only `localhost` is fine.
    # awk exits non-zero when there is no `localhost` alias to drop, so this
    # is a no-op on hosts files that never had the problem.
    if [ -w /etc/hosts ] && pinned=$(awk '
        $1 != "::1" { print; next }
        {
            line = $1
            names = 0
            for (i = 2; i <= NF; i++) {
                if ($i == "localhost") { dropped = 1; continue }
                line = line " " $i
                names++
            }
            if (names) print line   # a line with no names left is invalid
        }
        END { exit !dropped }
    ' /etc/hosts) && [ -n "$pinned" ]; then
        # `> /etc/hosts` rather than a temp file + mv: it is a bind mount, so it
        # must be written in place. The `||` keeps a write failure from aborting
        # the entrypoint under `set -e`.
        printf '%s\n' "$pinned" > /etc/hosts &&
            echo "Pinned 'localhost' to IPv4 in /etc/hosts" ||
            echo "WARNING: could not pin 'localhost' to IPv4; continuing"
    fi
