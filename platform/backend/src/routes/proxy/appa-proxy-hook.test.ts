...
describe("OpenAPPA durable proxy correlation", () => {
  ...
  test("keeps a client-local native MCP approval refusal out of APPA data", async () => {
    const events: Array<Record<string, unknown>> = [];
    server.use(
      http.post(hookUrl, async ({ request }) => {
        const event = (await request.json()) as Record<string, unknown>;
    ...
    const callId = "call_appa_0123456789abcdef0123456789abcdef";
    const arguments_ = '{"tool_name":"fixture_tool","tool_args":{}}';
    const first = await open({
      sessionId: "native-local-refusal",
      nativeCodexExecution: true,
    });
    await first.authorizeOutboundToolCalls([
      {
        id: callId,
        emittedName: "mcp__fixture_gateway__archestra__run_tool",
        emittedArguments: arguments_,
        emittedArgumentsCanonical: canonicalJsonObject(arguments_),
        targetName: "mcp/fixture_gateway/fixture_tool",
        targetArguments: {},
      },
    ]);
    await first.finish();

    const rawLocalFailure = "MCP tool call requires approval.";
    await expect(
      open({
        sessionId: "native-local-refusal",
        nativeCodexExecution: true,
        toolResults: [
          {
            id: callId,
            content: rawLocalFailure,
            claimedCall: {
              name: "mcp__fixture_gateway__archestra__run_tool",
    ...
    const result = events.findLast((event) => event.event === "tool_result");
    expect(result).toMatchObject({ outcome: { status: "indeterminate" } });
    expect(JSON.stringify(result)).not.toContain(rawLocalFailure);
  });
  ...
  test("admits only one parallel turn", async () => {
