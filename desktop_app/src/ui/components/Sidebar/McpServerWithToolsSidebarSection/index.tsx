...
import { useToolsStore, useMcpServersStore } from '@ui/stores';
...
export default function McpServerWithToolsSidebarSection(_props: McpServerWithToolsSidebarSectionProps) {
  ...
  const toolsByServer = availableTools
    ...
    .reduce((acc: Record<string, { tools: typeof availableTools; commonPrefix: string; serverId: string }>, tool) => {
  ...
  if (toolSearchQueryIsEmpty) {
    installedMcpServers.forEach(server => {
      ...
      const isInitializing = server.state === 'not_created' || 
      ...
      const serverAlreadyShowing = Object.values(toolsByServer).some(
        group => group.serverId === server.id
      );
                    ...
                    <SidebarMenuItem>
                      <div className="flex items-center gap-1">
                        <CollapsibleTrigger className="flex-1 min-w-0">
                          <div className={`flex items-center justify-between gap-2 px-2 py-1.5 bg-muted/50 rounded-md transition-colors w-full ${isInitializing && !isError ? 'opacity-60' : 'cursor-pointer hover:bg-muted/70'}`}>
                            <div className="flex items-center gap-2 min-w-0 flex-1">
                              ...
                              <span className="text-xs text-muted-foreground flex-shrink-0">
                                ({serverData.tools.length}{isInitializing && !isError && serverData.tools.length === 0 ? ' loading...' : isError ? ' error' : ''})
                              </span>
                            </div>
                            <div className="flex items-center gap-1 flex-shrink-0">
                              <button
                                ...
                                title={isError ? `${serverName} has an error` : isInitializing && serverData.tools.length === 0 ? `${serverName} is still initializing` : `Add all ${serverName} tools`}
                                disabled={isInitializing && serverData.tools.length === 0}
                              >
                                <PlusCircle className={`h-4 w-4 ${(isInitializing && serverData.tools.length === 0) ? 'text-muted-foreground/50' : 'text-muted-foreground hover:text-foreground'}`} />
                              </button>
                              {isExpanded ? (
                      ...
                      {serverData.tools.length === 0 ? (
                        <SidebarMenuItem>
                          <div className="px-4 py-2 text-xs text-muted-foreground italic">
                            {isError ? 'Server error - check Settings' : isInitializing ? 'Loading tools...' : 'No tools available'}
                          </div>
                        </SidebarMenuItem>
                      ) : (
                        ...
                        const displayName = serverData.commonPrefix ? fullName.slice(serverData.commonPrefix.length) : fullName;
                        ...
                        return (
                          <SidebarMenuItem key={id}>
                            <ToolHoverCard
                              ...
                              instructionText={isInitializing ? "Server is still initializing" : "Click to add this tool to your chat"}
                            >
                              <div 
                              ...
                              >
                                {status === 'awaiting_ollama_model' || status === 'in_progress' ? (
                                  ...
                                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
                                    tool.analysis?.is_read && tool.analysis?.is_write ? 'bg-blue-500' :
                                    tool.analysis?.is_write ? 'bg-orange-500' :
                                    tool.analysis?.is_read ? 'bg-green-500' :
                                    'bg-gray-500'
                                  }`} />
                                )}
