 ...
 * Kubernetes bumps it on every spec change and never on a metadata-only one,
 * so it counts scale events exactly and annotation writes cannot inflate it.
 */
test.describe("MCP idle hibernation - on-demand wake", () => {
  ...
  test("a tool call wakes the hibernated deployment, restoring its replicas and clearing both annotations", async ({
    ...
    // fast path (the stale-cache race is a known prototype limitation). A
    ...
    // And it got there in ONE scale event, the one this call caused: the wake
    // restores the replicas in a single spec change and clears the annotations
    // in a metadata-only patch, which Kubernetes never counts. A wake that
    // flapped the deployment down and up again would show two more.
    expect((await readDeploymentFacts()).generation).toBe(
      (asleep.generation ?? 0) + 1,
    );
  });
  ...
  test("simultaneous calls to one hibernated deployment share a single wake", async ({
    ...
    const facts = await readDeploymentFacts();
    // Exactly one scale-up for the whole burst: one spec change on the
    // Deployment (0 -> its recorded count) and one pod created by its
    // ReplicaSet. A wake per caller shows up as extra generations, extra pod
    // creations, or both — and a wake that hibernated and re-woke in between
    // would add two generations per cycle.
    expect(facts.generation).toBe(generationWhileHibernated + 1);
  ...
  test("a call finishes a half-woken deployment instead of starting a second wake", async ({
    ...
    // Finished, not restarted: the only scale-up in this window is the one
    // this test performed, and only the pod it produced ever existed. A
    // second wake would scale the deployment again (two more generations for
    // a down-and-up cycle) or replace the pod.
    expect((await readDeploymentFacts()).generation).toBe(halfWoken.generation);
