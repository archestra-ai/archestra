...
class AgentModel {
  ...
  static async update(
    ...
    if (Object.keys(agent).length > 0) {
      // For internal agents (agentType='agent'), automatically track version history
      // so that the version history feature works correctly.
      let updatePayload: typeof agent & {
