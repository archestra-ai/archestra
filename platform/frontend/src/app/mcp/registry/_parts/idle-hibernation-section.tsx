...
export function IdleHibernationSection({ item }: { item: CatalogItem }) {
  ...
  // The override lives on the installed server rows, and the reinstall route
  // can move ONE install's copy independently — so the control derives from
  // ALL of them, not whichever row lists first: one shared value renders as
  // itself, divergence renders as an explicit mixed state, and zero installs
  // disable the control (the catalog update cascades over install rows, and
  // over zero rows it changes nothing).
  const installs = servers.filter((server) => server.catalogId === item.id);
  const distinctModes = [
    ...new Set(
      installs.map(
        (server) => server.hibernationMode ?? DEFAULT_HIBERNATION_MODE,
      ),
    ),
  ];
  const mode = distinctModes.length === 1 ? distinctModes[0] : undefined;
            ...
            <SelectValue
              placeholder={
                installs.length === 0
                  ...
                  : "Mixed across installs — pick one to align them"
              }
            />
          </SelectTrigger>
          <SelectContent>
