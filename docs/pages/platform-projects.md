...
By default the list shows only projects they can access, so a `project:admin` reaches other members' projects with the **View as admin** toggle next to the filters. Turning it on lists every project in the organization — other members' private projects and team-shared ones the admin isn't a member of — in every scope, with a by-user picker to narrow to specific owners. In that oversight view a project admin can edit or delete the project, change its sharing, and view, download, or delete its files — but cannot see or start its chats: the Chats panel is hidden, and the project's conversations remain private to its members.
