...
mod tests {
    ...
    fn full_config_builds() {
        let policy = Policy::from_toml(FULL).unwrap();
        assert_eq!(policy.upstream_base_url, "https://openrouter.ai/api/v1");
        assert_eq!(policy.approval_tool, ToolName::new("baton__request_approval"));
        assert_eq!(policy.unknown_policy, UnknownPolicy::Escalate);
        assert_eq!(policy.taint_policy, TaintPolicy::Escalate);
        assert_eq!(policy.user_id, UserId::new("alice@archestra.ai"));
        assert_eq!(
            policy.user_label.audience,
            Audience::readers([UserId::new("alice@archestra.ai"), UserId::new("bob@archestra.ai")])
        );
        // Two declared tools + the injected approval tool.
        assert_eq!(policy.contracts.len(), 3);
        assert!(policy.has_contract(&ToolName::new("baton__request_approval")));
        assert_eq!(
            policy.recipients_args[&ToolName::new("send_email")],
            vec!["to".to_string(), "cc".to_string(), "bcc".to_string()]
        );
    }

    #[test]
    fn minimal_config_defaults() {
        let policy = Policy::from_toml(r#"upstream_base_url = "http://localhost:9000""#).unwrap();
        assert_eq!(policy.approval_tool, ToolName::new(DEFAULT_APPROVAL_TOOL));
        assert_eq!(policy.unknown_policy, UnknownPolicy::Escalate);
        assert_eq!(policy.taint_policy, TaintPolicy::Allow);
        assert_eq!(policy.user_label, Label::identity());
        // Only the injected approval contract.
    ...
    #[test]
    fn bad_audience_keyword_is_rejected() {
        let err = Policy::from_toml(
            r#"upstream_base_url = "x"
            [user]
            audience = "everyone""#,
        ...
        .unwrap_err();
        assert!(matches!(err, ConfigError::AudienceKeyword(k) if k == "everyone"));
    }

    #[test]
    fn a_tool_named_like_the_approval_tool_is_rejected() {
        let err = Policy::from_toml(
            r#"upstream_base_url = "x"
            [[tool]]
            name = "baton__request_approval""#,
        )
        .unwrap_err();
        assert!(matches!(err, ConfigError::ApprovalToolCollision(_)));
    }
    ...
    fn duplicate_tool_is_rejected() {
        ...
        .unwrap_err();
        assert!(matches!(err, ConfigError::DuplicateTool(t) if t == "send_email"));
    }

    #[test]
    fn public_and_unknown_audience_keywords() {
        ...
        let reader = policy
            .contracts
            .iter()
            .find(|c| c.name == ToolName::new("reader"))
            .unwrap();
