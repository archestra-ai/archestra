...
-- drizzle-migration-linter: reason=The schedule_triggers.project_id FK targets the all-NULL column added immediately above, so validation scans no existing rows and takes no blocking lock; the index is on that same just-added column. The add-validating-constraint / create-index rules do not apply here.
