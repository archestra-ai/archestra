...
Most tools require explicit assignment to Agents or MCP Gateways before they can be used. The following tools are pre-installed on all new agents by default: [`todo_write`](#todo_write), [`query_knowledge_sources`](#query_knowledge_sources), [`advisor`](#advisor).
...
| Tool | Description | Required RBAC Permission |
|------|-------------|--------------------------|
| `advisor` | Ask a stronger model for a second opinion before you commit to something. | None (no additional RBAC permission required) |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `question` | `string` | Yes | The decision or problem you want a second opinion on. |
| `context` | `string` | No | What the advisor needs to answer well: what you have already tried, the options you are weighing, constraints, and evidence. The advisor cannot see your conversation, files, or tools. |
