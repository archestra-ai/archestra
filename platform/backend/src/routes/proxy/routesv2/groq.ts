 ...
 * This module registers proxy routes for Groq chat completions.
 *
...
import { Groq, constructResponseSchema, UuidIdSchema } from "@/types";
...
const groqProxyRoutesV2: FastifyPluginAsyncZod = async (fastify) => {
  ...
  /**
   * Register HTTP proxy for Groq routes
  ...
  await fastify.register(fastifyHttpProxy, {
    ...
    preHandler: (request, reply, next) => {
      ...
      if (uuidMatch) {
        // Strip UUID: /v1/groq/:uuid/path -> /v1/groq/path
