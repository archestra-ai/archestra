...
To stop the bot replying in a thread, send `mute` (or `@mention` it with `mute`), or react to one of its replies with the mute (🔇) or shushing-face (🤫) emoji. It goes quiet until the thread is @mentioned again.
