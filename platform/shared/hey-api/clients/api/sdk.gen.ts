...
/**
 * Permanently delete a soft-deleted agent (admins only). Unlinks its LLM interaction history and destroys the row; the data cannot be recovered. A trash action, never a shortcut: 404 unless the agent is already soft-deleted.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * None (no additional RBAC permission required)
 */
export const purgeAgent = <ThrowOnError extends boolean = false>(options: Options<PurgeAgentData, ThrowOnError>) => (options.client ?? client).delete<PurgeAgentResponses, PurgeAgentErrors, ThrowOnError>({ url: '/api/agents/{id}/permanent', ...options });
...
/**
 * List all knowledge bases for the organization. `status=deleted` lists soft-deleted knowledge bases org-wide and requires the manage-deleted permission (granted to admins by default); `search` does not apply to that slice.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
...
/**
 * Restore a soft-deleted knowledge base (requires the manage-deleted permission). Agent assignments and connector links survive deletion, so the restored knowledge base is immediately live for previously-assigned agents. 404 if there is no soft-deleted knowledge base with that id in the org.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * Authorization:
 *
 * `knowledgeSource:manage-deleted`: View, restore, and permanently delete soft-deleted Knowledge Bases and Connectors
 */
...
/**
 * Permanently delete a soft-deleted knowledge base (requires the manage-deleted permission). Destroys the row and its assignments; the data cannot be recovered. A trash action, never a shortcut: 404 unless the knowledge base is already soft-deleted.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `knowledgeSource:manage-deleted`: View, restore, and permanently delete soft-deleted Knowledge Bases and Connectors
 */
export const purgeKnowledgeBase = <ThrowOnError extends boolean = false>(options: Options<PurgeKnowledgeBaseData, ThrowOnError>) => (options.client ?? client).delete<PurgeKnowledgeBaseResponses, PurgeKnowledgeBaseErrors, ThrowOnError>({ url: '/api/knowledge-bases/{id}/permanent', ...options });
...
/**
 * List all connectors for the organization. `status=deleted` lists soft-deleted connectors org-wide and requires the manage-deleted permission (granted to admins by default); the other filters do not apply to that slice.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
...
/**
 * Restore a soft-deleted connector (requires the manage-deleted permission). The stored credential was destroyed when the connector was deleted, so it is restored DISABLED — re-authenticate, then re-enable it to resume syncing. 404 if there is no soft-deleted connector with that id in the org.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
...
/**
 * Permanently delete a soft-deleted connector (requires the manage-deleted permission). Destroys its documents, chunks, runs, ACLs, and assignments; the data cannot be recovered. A trash action, never a shortcut: 404 unless the connector is already soft-deleted.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `knowledgeSource:manage-deleted`: View, restore, and permanently delete soft-deleted Knowledge Bases and Connectors
 */
export const purgeConnector = <ThrowOnError extends boolean = false>(options: Options<PurgeConnectorData, ThrowOnError>) => (options.client ?? client).delete<PurgeConnectorResponses, PurgeConnectorErrors, ThrowOnError>({ url: '/api/connectors/{id}/permanent', ...options });
...
/**
 * Permanently delete a soft-deleted project (admins only). Destroys its retained files (rows and stored bytes) and scheduled tasks; the data cannot be recovered. A trash action, never a shortcut: 404 unless the project is already soft-deleted.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 */
export const purgeProject = <ThrowOnError extends boolean = false>(options: Options<PurgeProjectData, ThrowOnError>) => (options.client ?? client).delete<PurgeProjectResponses, PurgeProjectErrors, ThrowOnError>({ url: '/api/projects/{id}/permanent', ...options });
...
/**
 * Permanently delete a soft-deleted skill (admins only). Destroys its versions and resource files; the data cannot be recovered. A trash action, never a shortcut: 404 unless the skill is already soft-deleted. 409 while a sandbox mount still pins one of its versions.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 */
export const purgeSkill = <ThrowOnError extends boolean = false>(options: Options<PurgeSkillData, ThrowOnError>) => (options.client ?? client).delete<PurgeSkillResponses, PurgeSkillErrors, ThrowOnError>({ url: '/api/skills/{id}/permanent', ...options });
