...
type XaiToolResultContentBlock = XaiToolResultImageBlock | XaiToolResultTextBlock;
...
class XaiRequestAdapter implements LLMRequestAdapter<XaiRequest, XaiMessages> {
  ...
  convertToolResultContent(messages: XaiMessages): XaiMessages {
    ...
    const result = messages.map((message) => {
      ...
      // Model supports images - convert MCP image blocks to x.ai format (OpenAI-compatible)
...
async function convertToolResultsToToon(
  ...
  const tokenizer = getTokenizer("openai"); // x.ai uses OpenAI-compatible tokenization
...
> = {
  ...
  extractApiKey(headers: XaiHeaders): string | undefined {
    ...
    // x.ai uses standard Bearer token authentication
  ...
  createClient(
    ...
    // x.ai uses OpenAI-compatible SDK
    return new OpenAIProvider({
      apiKey,
      baseURL: options?.baseUrl ?? config.llm.xai?.baseUrl ?? "https://api.x.ai/v1",
      fetch: customFetch,
    });
  },
