 ...
 *
 * x.ai uses an OpenAI-compatible API. This adapter reuses OpenAI's
 * request/response/stream adapters with x.ai-specific configuration.
 *
 * Since x.ai is OpenAI-compatible, we delegate all adapter logic to OpenAI
 * and only override provider-specific configuration (baseUrl, provider name, etc.).
 *
...
// TYPE ALIASES (reuse OpenAI-compatible x.ai types)
