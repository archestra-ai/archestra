...
export type AgentRuntimeEgressPolicyObject =
...
export const AGENT_RUNTIME_EGRESS_POLICY_CRDS = {
  CiliumNetworkPolicy: {
    group: "cilium.io",
    version: "v2",
...
/**
 * Apply the Agent Environment's effective egress policy to one run.
 *
...
export function buildAgentRuntimeEnvironmentEgressPolicies(params: {
  ...
  const podSelectorLabels = { [AGENT_RUNTIME_TASK_LABEL]: spec.taskId };
  const metadata = {
...
function withAgentRuntimeMetadata(params: {
  kind: AgentRuntimeEgressPolicyObject["kind"];
