...
# profile wrapper runs the real claude no matter how this script exits.
...
function Show-ArchDownPrompt($r) {
  Clear-ArchLine
...
function Wait-ArchHealth {
  ...
  while ($true) {
    ...
    $key = Read-ArchKey
    if ($key -eq 's' -or $key -eq 'S') { break }
    if ($key -eq 'd' -or $key -eq 'D') {
      Disconnect-ArchRemote $Remotes[0].Kind $Remotes[0].Label
      $Script:FirstHandled = $true
      break
    }
    $now = [DateTime]::UtcNow
    ...
    if ($elapsed -ge $HangTightAfterSeconds) {
      Show-ArchSpin $Remotes[0].Label ('— trying to connect... ' + $elapsed + 's, few more seconds, hang tight...  [s] skip  [d] disconnect')
    } elseif ($elapsed -ge $NoticeAfterSeconds) {
      Show-ArchSpin $Remotes[0].Label ('— trying to connect... ' + $elapsed + 's  [s] skip  [d] disconnect')
    }
  }
...
if ($HealthUrl) {
  Show-ArchSpin $Remotes[0].Label ''
  Wait-ArchHealth
}
$archIndex = -1
foreach ($r in $Remotes) {
  $archIndex++
  if ($archIndex -eq 0 -and $Script:FirstHandled) { continue }
  Show-ArchSpin $r.Label ''
  for ($pad = 0; $pad -lt $MinCheckFrames; $pad++) {
    Start-Sleep -Milliseconds $FrameSleepMs
    Show-ArchSpin $r.Label ''
  }
  if (Test-ArchResourceDown $r) { Show-ArchDownPrompt $r }
