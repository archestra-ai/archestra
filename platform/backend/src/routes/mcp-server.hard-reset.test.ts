...
} from "@/k8s/mcp-server-runtime/hard-reset-status";
...
class FakeCluster {
  ...
  readDeployment(name: string): k8s.V1Deployment {
    ...
    return {
      metadata: {
        ...
        annotations: {},
        resourceVersion: "1",
        uid: `deployment-${this.deploymentRevision}`,
      },
  ...
  deleteDeployment(name: string, expectedUid?: string): void {
    if (
      expectedUid &&
      expectedUid !== `deployment-${this.deploymentRevision}`
    ) {
      throw { statusCode: 409, message: "UID precondition failed" };
    }
  ...
  createDeployment(name: string, podLabels: Record<string, string>): void {
    ...
    this.pods.set(`${name}-7c5f8d9a1-fghij`, {
      ...
      uid: `pod-${name}-${this.deploymentRevision}`,
  ...
  deletePod(
    ...
    if (expectedUid && this.pods.get(name)?.uid !== expectedUid) {
      throw { statusCode: 409, message: "UID precondition failed" };
    }
...
type ManagerInternals = {
  ...
  hardResetPhysicalKeyByServerId: Map<string, string>;
  wakeInFlightByPhysicalKey: Map<string, Promise<void>>;
  reconcileInterruptedHardResets: () => Promise<void>;
};
...
function useFakeCluster(cluster: FakeCluster, deploymentDelay?: Promise<void>) {
  const coreApi = {
    ...
    deleteNamespacedPod: vi.fn(
      async ({
        ...
        cluster.deletePod(name, gracePeriodSeconds, body?.preconditions?.uid);
        return {};
      },
    ),
  ...
  const appsApi = {
    ...
    deleteNamespacedDeployment: vi.fn(
      async ({ name, body }: { name: string; body?: k8s.V1DeleteOptions }) => {
        cluster.deleteDeployment(name, body?.preconditions?.uid);
  ...
  internals.hardResetPhysicalKeyByServerId.clear();
  internals.wakeInFlightByPhysicalKey.clear();
  return internals;
}
...
describe("POST /api/mcp_server/:id/hard-reset", () => {
  ...
  test("does not rebuild when the old pod teardown cannot be verified", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const { mcpServer } = await makeLocalInstall({
    ...
    // The Deployment is already gone by the time the pod check fails, but an
    // old pod may still be serving behind the reused Service selector.
    ...
    expect(res.json().recreated).toBe(false);
    expect(cluster.createdDeployments).toEqual([]);
    expect(res.json().rebuild).toEqual({
      outcome: "not-ready",
      reason: expect.stringContaining("rebuild was skipped"),
    });
  ...
  test("does not wait on a wake queued behind the reset transition lease", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const internals = useFakeCluster(cluster);
    const { mcpServer } = await makeLocalInstall({
      makeInternalMcpCatalog,
      makeMcpServer,
    });
  ...
  test("a stuck pod whose force-delete fails is reported unverified, never force-killed", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const { mcpServer } = await makeLocalInstall({
    ...
    // A finalizer-stuck pod: it survives the Deployment's delete cascade, and
    // the cluster then refuses the escalation outright.
    const originalDeleteDeployment = cluster.deleteDeployment.bind(cluster);
    cluster.deleteDeployment = (name) => {
      originalDeleteDeployment(name);
      cluster.addPod({
        name: POD_NAME,
    ...
    cluster.deletePod = () => {
      throw { statusCode: 403, message: `pods "${POD_NAME}" is forbidden` };
    };

    // Faked before the reset starts so both pod-grace windows are faked too.
    vi.useFakeTimers({ toFake: ["setTimeout"] });
    const handle = await McpServerRuntimeManager.hardResetDeployment(
      mcpServer.id,
    );
    handle.acknowledgePending();
    let result: Awaited<typeof handle.completion> | undefined;
    void handle.completion.then((r) => {
      result = r;
    });
    for (let seconds = 0; seconds < 300 && !result; seconds++) {
      await vi.advanceTimersByTimeAsync(1_000);
    }
    vi.useRealTimers();
    if (!result) throw new Error("the reset never completed");
    handle.acknowledgeReporting();

    // The old pod may still be alive behind the reused Service selector, so
    // "force-killed" would claim a clean teardown the cluster never showed.
    expect(result.teardown).toEqual({
      outcome: "unverified",
      reason: expect.stringContaining(POD_NAME),
    });
  ...
  test("a stuck pod is only reported force-killed once a re-listing shows it gone", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const { mcpServer } = await makeLocalInstall({
    ...
    // Survives the cascade; the explicit gracePeriodSeconds:0 delete works.
    const originalDeleteDeployment = cluster.deleteDeployment.bind(cluster);
    cluster.deleteDeployment = (name) => {
      originalDeleteDeployment(name);
      cluster.addPod({
    ...
    expect(cluster.deletedPods).toContainEqual({
      name: POD_NAME,
      gracePeriodSeconds: 0,
    });
    expect(result.rebuild).toEqual({ outcome: "ready" });
  });
  ...
  test("resets a multitenant catalog's shared deployment once, for every install on it", async ({
    ...
    // The verdict lands on every install the shared deployment backs, not
    // just the one whose id was in the URL.
    for (const id of [first.id, second.id]) {
      const row = await readInstall(id);
      expect(row?.localInstallationStatus).toBe("success");
      expect(row?.localInstallationError).toBeNull();
    }
  });

  test("a reset already running on another replica answers in-progress and destroys nothing", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const { mcpServer } = await makeLocalInstall({
    ...
    // Another web replica's reset, as this replica can see it: a live lease
    // row on the physical deployment. The in-process single-flight knows
    // nothing about it — without the lease this replica would tear down the
    // Deployment the other one is mid-way through rebuilding.
    await db.insert(schema.mcpDeploymentLeasesTable).values({
      scope: "mcp-hard-reset",
    ...
    // The same truthful shape as a reset that outlives the response budget:
    // the reset IS running — elsewhere — and its outcome lands on the
    // install statuses.
    expect(res.statusCode).toBe(200);
    expect(res.json()).toEqual({
      status: "in-progress",
    ...
    expect(cluster.createdDeployments).toEqual([]);
    expect(cluster.deploymentExists).toBe(true);
    // This observer did not start the reset and must not overwrite status the
    // owning replica controls.
    expect((await readInstall(mcpServer.id))?.localInstallationStatus).toBe(
      "error",
    );

    // With the owner gone (lease released), the next request resets for real.
    await db
  ...
  test("a reset refuses to start when the shared blast radius cannot be determined", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const catalog = await makeInternalMcpCatalog({
    ...
    // One transient DB error while resolving the sibling set. With the old
    // caller-only fallback this collapsed the affected set to the requester
    // and the shared pod was destroyed anyway, leaving the sibling's status,
    // sessions and caches claiming a pod that no longer existed.
    vi.spyOn(McpServerModel, "findByCatalogId").mockRejectedValueOnce(
    ...
    expect(res.json().error.message).toContain("database connection lost");
    // Nothing was destroyed or rebuilt.
    expect(cluster.deletedDeployments).toEqual([]);
    expect(cluster.createdDeployments).toEqual([]);
    expect(cluster.deploymentExists).toBe(true);
    // Neither row changes: the reset never owned a destructive phase, and a
    // late failure write could overwrite a newer replica's pending marker.
    expect((await readInstall(first.id))?.localInstallationStatus).toBe(
      "success",
    );
    expect((await readInstall(first.id))?.localInstallationError).toBeNull();
    const sibling = await readInstall(second.id);
    expect(sibling?.localInstallationStatus).toBe("success");
    expect(sibling?.localInstallationError).toBeNull();
  });

  test("a shared rebuild that stays down is recorded on every sibling install", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const catalog = await makeInternalMcpCatalog({
    ...
    // The destroyed deployment was every sibling's backing pod. A sibling row
    // still reading "success" would be a healthy-looking server that
  ...
  test("transient batch status failures retry without splitting sibling outcomes", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const catalog = await makeInternalMcpCatalog({
    ...
    const originalUpdate =
      McpServerModel.updateInstallationStatuses.bind(McpServerModel);
    let pendingWriteFailed = false;
    let finalWriteFailed = false;
    vi.spyOn(McpServerModel, "updateInstallationStatuses").mockImplementation(
      async (params) => {
        if (!pendingWriteFailed && params.status === "pending") {
          pendingWriteFailed = true;
          throw new Error("transient status write failure");
        }
        if (!finalWriteFailed && params.status === "success") {
          finalWriteFailed = true;
          throw new Error("transient final status write failure");
        }
    ...
    // One SQL statement changes the whole sibling set, so every retry lands an
    // all-or-none status rather than leaving a healthy-looking sibling behind.
    for (const id of [first.id, second.id]) {
      const row = await readInstall(id);
      expect(row?.localInstallationStatus).toBe("success");
      expect(row?.localInstallationError).toBeNull();
  ...
  test("persistent pending-status failure prevents destructive reset work", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const { mcpServer } = await makeLocalInstall({
    ...
    const row = await readInstall(mcpServer.id);
    expect(row?.localInstallationStatus).toBe("error");
    expect(row?.localInstallationError).toContain("status store unavailable");
  });

  test("reconciles a durable pending marker after final reporting fails", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const { mcpServer } = await makeLocalInstall({
    ...
    const originalUpdate =
      McpServerModel.updateInstallationStatuses.bind(McpServerModel);
    const updateSpy = vi
      .spyOn(McpServerModel, "updateInstallationStatuses")
      .mockImplementation(async (params) => {
        if (params.status === "success") {
          throw new Error("final status store unavailable");
        }
        return originalUpdate(params);
      });
    ...
    expect(stranded?.localInstallationStatus).toBe("pending");
    expect(stranded?.localInstallationError).toMatch(
      new RegExp(`^${HARD_RESET_STATUS_MARKER_PREFIX}`),
    );
    expect(
      parseHardResetStatusMarker(stranded?.localInstallationError ?? null)
        ?.phase,
    ).toBe("teardown-complete");

    updateSpy.mockRestore();
    await (
      McpServerRuntimeManager as unknown as ManagerInternals
    ).reconcileInterruptedHardResets();

    const recovered = await readInstall(mcpServer.id);
    expect(recovered?.localInstallationStatus).toBe("success");
    expect(recovered?.localInstallationError).toBeNull();
  });

  test("does not credit an external replacement before reset teardown", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    const { mcpServer } = await makeLocalInstall({
    ...
    const originalUid = cluster.readDeployment(DEPLOYMENT_NAME).metadata?.uid;
    const marker = createHardResetStatusMarker({
      physicalDeployment: `${NAMESPACE}/${DEPLOYMENT_NAME}`,
      originalDeploymentUid: originalUid,
    });
    ...
    cluster.deleteDeployment(DEPLOYMENT_NAME, originalUid);
    cluster.createDeployment(DEPLOYMENT_NAME, {
      app: "mcp-server",
      "mcp-server-id": mcpServer.id,
    });
    ...
    const untouched = await readInstall(mcpServer.id);
    expect(untouched?.localInstallationStatus).toBe("pending");
    expect(untouched?.localInstallationError).toBe(marker);
  });

  test("siblings move to pending while a shared reset is under way", async ({
    makeInternalMcpCatalog,
    makeMcpServer,
  }) => {
    ...
    const statusBroadcast = vi.spyOn(
      websocketService,
    ...
    // While the reset is parked inside its teardown, an administrator looking
    // at the SIBLING must already see it in flux — its pod is being destroyed.
    await waitUntil(() =>
      statusBroadcast.mock.calls.some(
        ([serverId, status]) => serverId === second.id && status === "pending",
      ),
    );
  ...
  test("two concurrent resets share one teardown", async ({
    ...
    // Start the second request immediately, before the first reaches any K8s
    // write. The in-process entry must be installed without an await gap.
  ...
  test("reports a reset that outlives the request, and lands its outcome on the install", async ({
    ...
    expect(statusBroadcast).toHaveBeenCalledWith(
      mcpServer.id,
      "pending",
      expect.stringMatching(new RegExp(`^${HARD_RESET_STATUS_MARKER_PREFIX}`)),
    );
