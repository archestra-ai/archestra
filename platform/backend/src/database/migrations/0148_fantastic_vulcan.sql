...
-- Data migration: existing keys were unique per provider+scope, so they are all primary
UPDATE "chat_api_keys" SET "is_primary" = true;--> statement-breakpoint
...
CREATE INDEX "idx_virtual_api_key_token_start" ON "virtual_api_keys" USING btree ("token_start");