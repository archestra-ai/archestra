...
describe("buildAgentRuntimeEnvironmentEgressPolicies", () => {
  it("gives the built-in Environment public egress without private-network access", () => {
    const [policy] = buildAgentRuntimeEnvironmentEgressPolicies({
      spec: SPEC,
    ...
    expect(object.metadata?.name).toBe("agent-run-deploy-app-11111111-egress");
  ...
  it("uses Cilium for a domain allowlist when the cluster supports it", () => {
    ...
    expect(policies[0]?.object).toMatchObject({
      ...
      spec: {
        endpointSelector: {
          matchLabels: { [AGENT_RUNTIME_TASK_LABEL]: SPEC.taskId },
        },
  ...
  it("uses AWS ApplicationNetworkPolicy for restricted, blocked, and built-in egress", () => {
    ...
    const restricted = buildAgentRuntimeEnvironmentEgressPolicies({
      spec: restrictedSpec(),
      capabilities,
      clusterDnsIps: ["172.20.0.10"],
    });
    const builtIn = buildAgentRuntimeEnvironmentEgressPolicies({
      spec: SPEC,
      capabilities,
      clusterDnsIps: ["172.20.0.10"],
    });
    const blocked = buildAgentRuntimeEnvironmentEgressPolicies({
      spec: {
        ...SPEC,
        effectiveNetworkPolicy: {
          source: "environment",
...
function restrictedSpec(): KubernetesAgentRunLaunchSpec {
  return {
    ...SPEC,
    effectiveNetworkPolicy: {
      source: "environment",
