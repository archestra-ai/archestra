 ...
 * - src/routes/proxy/adapterV2/minimax.ts
 *
 * V2 routes are located at:
 * - src/routes/proxy/routesv2/minimax.ts
 *
...
export function toCommonFormat(messages: OpenAiMessages): CommonMessage[] {
  logger.debug(
    ...
    "[adapters/minimax] toCommonFormat: starting conversion",
  );
    ...
    if (message.role === "tool") {
      ...
      if (toolName) {
        logger.debug(
          ...
          "[adapters/minimax] toCommonFormat: found tool message",
        );
  ...
  logger.debug(
    ...
    "[adapters/minimax] toCommonFormat: conversion complete",
  );
...
export function applyUpdates(
  ...
  logger.debug(
    ...
    "[adapters/minimax] applyUpdates: starting",
  );

  if (updateCount === 0) {
    logger.debug("[adapters/minimax] applyUpdates: no updates to apply");
    return messages;
  }
  ...
  const result = messages.map((message) => {
    if (message.role === "tool" && updates[message.tool_call_id]) {
      ...
      logger.debug(
        ...
        "[adapters/minimax] applyUpdates: applying update to tool message",
      );
      return {
        ...message,
  ...
  logger.debug(
    ...
    "[adapters/minimax] applyUpdates: complete",
  );
  return result;
}
