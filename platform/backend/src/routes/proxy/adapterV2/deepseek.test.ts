...
function createMockResponse(
  message: DeepSeek.Types.ChatCompletionsResponse["choices"][0]["message"],
  usage?: Partial<DeepSeek.Types.Usage>,
): DeepSeek.Types.ChatCompletionsResponse {
  return {
    id: "chatcmpl-test",
...
function createMockRequest(
  messages: DeepSeek.Types.ChatCompletionsRequest["messages"],
  options?: Partial<DeepSeek.Types.ChatCompletionsRequest>,
): DeepSeek.Types.ChatCompletionsRequest {
  return {
    model: "deepseek-chat",
    messages,
    ...options,
...
describe("DeepSeekResponseAdapter", () => {
  describe("getToolCalls", () => {
    test("converts function tool calls to common format", () => {
      ...
      const adapter = deepseekAdapterFactory.createResponseAdapter(response);
...
describe("DeepSeekRequestAdapter", () => {
  describe("getModel", () => {
    test("returns original model by default", () => {
      ...
      const adapter = deepseekAdapterFactory.createRequestAdapter(request);
    ...
    test("returns modified model after setModel", () => {
      ...
      const adapter = deepseekAdapterFactory.createRequestAdapter(request);
      adapter.setModel("deepseek-reasoner");
  ...
  describe("toProviderRequest - general", () => {
    ...
    test("converts MCP image blocks in tool results", () => {
      ...
      try {
        ...
        ] as unknown as DeepSeek.Types.ChatCompletionsRequest["messages"];
...
describe("deepseekAdapterFactory", () => {
  describe("extractApiKey", () => {
    ...
    test("returns undefined when no authorization header", () => {
      const headers = {} as unknown as DeepSeek.Types.ChatCompletionsHeaders;
      const apiKey = deepseekAdapterFactory.extractApiKey(headers);
  ...
  describe("provider info", () => {
    test("has correct provider name", () => {
      expect(deepseekAdapterFactory.provider).toBe("deepseek");
    });

    test("has correct interaction type", () => {
      expect(deepseekAdapterFactory.interactionType).toBe(
