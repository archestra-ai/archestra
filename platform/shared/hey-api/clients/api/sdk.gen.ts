...
/**
 * The project's memory entries (newest first) — short durable notes the assistant saved for the project. Every entry is injected into the system prompt of the project's chats. Readable by anyone with project access.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `project:read`: View projects and the chats inside them
 */
export const getProjectMemories = <ThrowOnError extends boolean = false>(options: Options<GetProjectMemoriesData, ThrowOnError>) => (options.client ?? client).get<GetProjectMemoriesResponses, GetProjectMemoriesErrors, ThrowOnError>({ url: '/api/projects/{id}/memories', ...options });

/**
 * Save one memory entry to the project. Any project member (owner or shared) may save; the per-project entry cap returns 409.
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `project:read`: View projects and the chats inside them
 */
export const createProjectMemory = <ThrowOnError extends boolean = false>(options: Options<CreateProjectMemoryData, ThrowOnError>) => (options.client ?? client).post<CreateProjectMemoryResponses, CreateProjectMemoryErrors, ThrowOnError>({
    url: '/api/projects/{id}/memories',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
 ...
 * `project:read`: View projects and the chats inside them
 */
export const deleteProjectMemory = <ThrowOnError extends boolean = false>(options: Options<DeleteProjectMemoryData, ThrowOnError>) => (options.client ?? client).delete<DeleteProjectMemoryResponses, DeleteProjectMemoryErrors, ThrowOnError>({ url: '/api/projects/{id}/memories/{memoryId}', ...options });

/**
 * Replace one memory entry's content. Any project member may edit any entry (memories are collaborative, not per-author).
 *
 * Authentication:
 *
 * Required. Use an authenticated browser session or send your Archestra API key in the `Authorization` header.
 ...
 * `project:read`: View projects and the chats inside them
 */
export const updateProjectMemory = <ThrowOnError extends boolean = false>(options: Options<UpdateProjectMemoryData, ThrowOnError>) => (options.client ?? client).patch<UpdateProjectMemoryResponses, UpdateProjectMemoryErrors, ThrowOnError>({
    url: '/api/projects/{id}/memories/{memoryId}',
    ...options,
    headers: {
        'Content-Type': 'application/json',
        ...options.headers
