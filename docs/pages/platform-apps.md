...
Two built-in tools can also be assigned to an app: `search_files` and `read_file`, the platform's read-only file tools. They appear as their own group in the app's tool assignment. Once granted, the app calls them like any other tool, and they read the files of **the chat the app is rendered in** — the conversation's files, or the whole project's files when that chat belongs to a project — always as the viewing user. This is what lets an app visualize files created in the conversation (for example, render a 3D model the agent just generated). On the standalone `/a/:id` page there is no chat, so the tools return no files. The app cannot create, change, or delete files, and file contents are not stored in the tool-call logs.
