...
export function detectProviderFromModel(model: string): SupportedChatProvider {
  ...
  if (lowerModel.includes("openrouter") || lowerModel.includes("llama") || lowerModel.includes("mixtral") || lowerModel.includes("gemma")) {
    return "openrouter";
  }
...
export const FAST_MODELS: Record<SupportedChatProvider, string> = {
  ...
  openrouter: "openai/gpt-4o-mini", // OpenRouter's fast versatile model
