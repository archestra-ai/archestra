...
Reviewed the local OpenAPPA UI on 2026-09-22 against `eef5600b2b`. Browser checks were read-only or used unsaved form drafts. No deployment setting, policy revision, GitHub source, battery, or database row was changed. Findings below describe the original experience; this branch fixes the draft-loss, validation, and first-run path issues. The deployment switch and legacy migration flow are deferred.

## P1 — Policy drafts disappear during ordinary in-app navigation

**Status:** Fixed in this branch for in-app links. Full page unloads remain covered by the existing browser warning.
...
**Status:** Deferred. The deployment switch is temporary.
...
**Status:** Fixed in this branch. The battery upload dialog gets the same discard guard.
...
**Status:** Deferred until the legacy guardrails migration. The current OpenAPPA page now links to the separate Claude Code plugin for clarity.
...
**Status:** Fixed in this branch. A failed validation is shown above the editor and disables save until the draft changes.
...
**Status:** Improved in this branch with policy and battery guides, an MCP Registry link, available battery descriptions, and a selected-file preview.
