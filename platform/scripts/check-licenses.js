...
try {
  ...
  console.warn("Warning: Could not load license-resolution.json, using empty verified list");
}
...
try {
  ...
  for (const { name, version, license, category } of results) {
    const baseCategory = category.replace(" (verified)", "").replace(" (dual)", "");
    const isProblematic = ["GPL", "AGPL", "Unknown", "Other"].includes(baseCategory) ||
                          (baseCategory.startsWith("Multi:") && /GPL/i.test(baseCategory) && !/LGPL/i.test(baseCategory));
    const warning = isProblematic ? "⚠️  " : "";
    const tag = `[${category}]`;
    console.log(`${warning}${tag.padEnd(25)} ${name}@${version} - ${license}`);
  }
