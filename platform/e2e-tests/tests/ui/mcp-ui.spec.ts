...
test.describe("MCP UI End-to-End Test", () => {
  test.beforeAll(async ({ request, installMcpServer, makeApiRequest }) => {
    ...
    if (!catalogItem1)
      throw new Error(`Catalog item '${MCP_UI_SERVER_1}' not found.`);
    const server1 = await findInstalledServer(request, catalogItem1.id);
    ...
    if (!catalogItem2)
      throw new Error(`Catalog item '${MCP_UI_SERVER_2}' not found.`);
    let server2 = await findInstalledServer(request, catalogItem2.id);
    if (!server2) {
      const res = await installMcpServer(request, {
        name: MCP_UI_SERVER_2,
    ...
    const tool1 = tools.find((t: { name: string }) =>
      t.name.includes(MCP_UI_SERVER_1),
    );
    const tool2 = tools.find((t: { name: string }) =>
      t.name.includes(MCP_UI_SERVER_2),
    );
    if (!tool1 || !tool2)
      throw new Error("Could not find the newly installed tools.");
        ...
        assignments: [
          {
            ...
            toolId: tool1.id,
            executionSourceMcpServerId: server1.id,
          },
          {
            agentId: defaultProfile.id,
            toolId: tool2.id,
            executionSourceMcpServerId: server2.id,
          },
        ],
      },
    });
